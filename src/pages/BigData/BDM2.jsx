...
const GraphDiagram = () => (
  <div style={S.diagram}>
    ...
    <svg viewBox="0 0 480 260" style={{ maxWidth: '100%', height: 'auto' }}>
      {/* Alice node */}
      <circle cx="100" cy="130" r="38" fill="rgba(74,158,237,0.12)" stroke="#4a9eed" strokeWidth="2" />
      <text x="100" y="125" textAnchor="middle" fill="#4a9eed" fontSize="11" fontWeight="800">Pessoa</text>
      ...
      {/* Bob node */}
      <circle cx="250" cy="60" r="38" fill="rgba(74,158,237,0.12)" stroke="#4a9eed" strokeWidth="2" />
      <text x="250" y="55" textAnchor="middle" fill="#4a9eed" fontSize="11" fontWeight="800">Pessoa</text>
      ...
      {/* Company node */}
      <circle cx="380" cy="130" r="38" fill="rgba(186,230,253,0.12)" stroke="#bae6fd" strokeWidth="2" />
      <text x="380" y="125" textAnchor="middle" fill="#bae6fd" fontSize="11" fontWeight="800">Empresa</text>
      ...
      {/* Movie node */}
      <circle cx="250" cy="210" r="38" fill="rgba(125,211,252,0.12)" stroke="#7dd3fc" strokeWidth="2" />
      <text x="250" y="205" textAnchor="middle" fill="#7dd3fc" fontSize="11" fontWeight="800">Filme</text>
      ...
      {/* Edges */}
      <line x1="136" y1="110" x2="214" y2="76" stroke="#4a9eed" strokeWidth="1.5" markerEnd="url(#arrow)" />
      <text x="160" y="87" textAnchor="middle" fill="#4a9eed" fontSize="9" fontWeight="700">CONHECE</text>
      <line x1="286" y1="76" x2="344" y2="110" stroke="#bae6fd" strokeWidth="1.5" markerEnd="url(#arrow2)" />
      <text x="343" y="87" textAnchor="middle" fill="#bae6fd" fontSize="9" fontWeight="700">TRABALHA_EM</text>
      <line x1="136" y1="150" x2="214" y2="194" stroke="#7dd3fc" strokeWidth="1.5" markerEnd="url(#arrow3)" />
      <text x="165" y="183" textAnchor="middle" fill="#7dd3fc" fontSize="9" fontWeight="700">VIU</text>
      <line x1="286" y1="194" x2="344" y2="150" stroke="#7dd3fc" strokeWidth="1.5" markerEnd="url(#arrow4)" />
      <text x="343" y="183" textAnchor="middle" fill="#7dd3fc" fontSize="9" fontWeight="700">PRODUZIU</text>
      <defs>
        <marker id="arrow" markerWidth="8" markerHeight="8" refX="7" refY="3" orient="auto"><path d="M0,0 L0,6 L8,3 z" fill="#4a9eed" /></marker>
        <marker id="arrow2" markerWidth="8" markerHeight="8" refX="7" refY="3" orient="auto"><path d="M0,0 L0,6 L8,3 z" fill="#bae6fd" /></marker>
        <marker id="arrow3" markerWidth="8" markerHeight="8" refX="7" refY="3" orient="auto"><path d="M0,0 L0,6 L8,3 z" fill="#7dd3fc" /></marker>
        <marker id="arrow4" markerWidth="8" markerHeight="8" refX="7" refY="3" orient="auto"><path d="M0,0 L0,6 L8,3 z" fill="#7dd3fc" /></marker>
      </defs>
    </svg>
  ...
  return (
    <div style={S.diagram}>
      ...
      <div style={{ display: 'flex', gap: '0.4rem', justifyContent: 'center', marginBottom: '1.25rem', flexWrap: 'wrap' }}>
        {metrics.map((mt, i) => (
          <button key={i} onClick={() => setMetric(i)} style={{ padding: '0.35rem 0.75rem', borderRadius: 20, cursor: 'pointer', fontWeight: 600, fontSize: '0.78rem', background: metric === i ? '#4a9eed' : 'var(--bg-primary)', color: metric === i ? '#000' : 'var(--text-primary)', border: `1.5px solid ${metric === i ? '#4a9eed' : 'var(--card-border)'}`, transition: 'all 0.2s' }}>{mt.name}</button>
        ))}
      </div>
      <div style={{ background: 'var(--bg-primary)', borderRadius: 10, padding: '1.25rem', textAlign: 'left', border: '1.5px solid rgba(74,158,237,0.25)' }}>
        <div style={{ display: 'inline-block', background: 'rgba(74,158,237,0.12)', border: '1px solid rgba(74,158,237,0.3)', color: '#4a9eed', fontSize: '0.8rem', fontWeight: 700, padding: '0.2rem 0.7rem', borderRadius: 12, marginBottom: '0.75rem', fontFamily: 'monospace' }}>{m.formula}</div>
        <p style={{ fontSize: '0.88rem', color: 'var(--text-primary)', marginBottom: '0.6rem' }}>{m.desc}</p>
        <p style={{ fontSize: '0.83rem', color: '#4a9eed', fontWeight: 600, marginBottom: '0.3rem' }}>Caso de uso: <span style={{ color: 'var(--text-secondary)', fontWeight: 400 }}>{m.use}</span></p>
        <p style={{ fontSize: '0.83rem', color: '#bae6fd', fontWeight: 600, marginBottom: 0 }}>Exemplo: <span style={{ color: 'var(--text-secondary)', fontWeight: 400 }}>{m.example}</span></p>
      </div>
    </div>
  );
};
    ...
    <div style={{ padding: '2rem 1rem' }}>
      <div style={S.page}>
            ...
            <table style={S.table}>
              ...
              <tbody>
                {[
                  ...
                  <tr key={p}><td style={{ ...S.td, fontWeight: 600 }}>{p}</td><td style={S.td}>{r}</td><td style={{ ...S.td, color: '#4a9eed' }}>{g}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
        ...
        <div style={S.section}>
          ...
          <p style={S.p}>Neo4j é a base de dados em grafo mais usada. Usa a linguagem de query <strong>Cypher</strong> — equivalente ao SQL mas para grafos. A sintaxe central é: <code style={{ background: 'rgba(74,158,237,0.1)', padding: '0.1rem 0.4rem', borderRadius: 4, fontSize: '0.9rem', color: '#4a9eed' }}>(nodes)-[:RELACAO]-{'>'} (otherNodes)</code></p>
            ...
            <table style={S.table}>
              ...
              <tbody>
                {[
                  ...
                  <tr key={op}><td style={{ ...S.td, fontWeight: 600, fontSize: '0.85rem' }}>{op}</td><td style={{ ...S.td, fontFamily: 'monospace', fontSize: '0.78rem', color: '#4a9eed' }}>{cy}</td><td style={{ ...S.td, fontSize: '0.82rem', color: 'var(--text-secondary)' }}>{sq}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
            {[
              ...
              <div key={title} style={{ background: 'var(--bg-secondary)', border: '1px solid rgba(74,158,237,0.2)', borderRadius: 10, padding: '1rem' }}>
                <div style={{ fontWeight: 700, color: '#4a9eed', marginBottom: '0.4rem', fontSize: '0.9rem' }}>{title}</div>
          ...
          <div style={S.note}>
            Exemplo real — Panama Papers: a query <code style={{ color: '#4a9eed' }}>MATCH path=(regula:Officer)--{'>'} () WHERE toUpper(regula.name) CONTAINS "REGULA LIMITED" RETURN path</code> devolve todas as relações de entidades financeiras offshore em segundos, sobre milhões de registos — o que em SQL relacional seria impraticável.
          </div>
        </div>
        ...
        <div style={S.section}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1.5rem' }}>
            <div style={{ background: 'var(--bg-secondary)', border: '1px solid rgba(74,158,237,0.2)', borderRadius: 10, padding: '1rem' }}>
              <div style={{ fontWeight: 700, color: '#4a9eed', marginBottom: '0.5rem' }}>Taxonomia</div>
            ...
            <div style={{ background: 'var(--bg-secondary)', border: '1px solid rgba(74,158,237,0.2)', borderRadius: 10, padding: '1rem' }}>
              <div style={{ fontWeight: 700, color: '#4a9eed', marginBottom: '0.5rem' }}>Ontologia</div>
              <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', margin: 0 }}>Tal como as taxonomias, mas não restrita a relações hierárquicas. Inclui relações como <code style={{ color: '#4a9eed' }}>part_of</code>, <code style={{ color: '#4a9eed' }}>compatible_with</code>, <code style={{ color: '#4a9eed' }}>depends_on</code>. Permite explorar categorias vertical e horizontalmente.</p>
            </div>
          </div>
            ...
            <table style={S.table}>
              ...
              <tbody>
                {[
                  ...
                  <tr key={a}><td style={{ ...S.td, fontWeight: 600 }}>{a}</td><td style={S.td}>{v}</td><td style={{ ...S.td, color: '#4a9eed' }}>{k}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
