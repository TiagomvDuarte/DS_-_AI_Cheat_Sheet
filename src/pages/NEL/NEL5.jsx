...
const PerceptronDiagram = () => (
  <div style={{ ...S.diagram, textAlign: 'center' }}>
    ...
    <svg viewBox="0 0 440 130" style={{ maxWidth: '100%', height: 'auto' }}>
      <defs>
        <marker id="arr-p" markerWidth="6" markerHeight="6" refX="5" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill="#4a9eed"/></marker>
      </defs>
      {[
        { y: 25, label: 'x₁', w: 'w₁' },
        { y: 55, label: 'x₂', w: 'w₂' },
        ...
        <g key={y}>
          <circle cx={30} cy={y} r={14} fill="rgba(74,158,237,0.10)" stroke="#4a9eed" strokeWidth="1.5"/>
          <text x={30} y={y + 4} textAnchor="middle" fill="#4a9eed" fontSize="9" fontWeight="700">{label}</text>
          <line x1={44} y1={y} x2={168} y2={70} stroke="#4a9eed" strokeWidth="1.2" markerEnd="url(#arr-p)"/>
          <text x={90} y={(y + 70) / 2 - 5} fill="var(--text-secondary)" fontSize="8" textAnchor="middle">{w}</text>
        </g>
      ))}
      <circle cx={190} cy={70} r={24} fill="rgba(74,158,237,0.10)" stroke="#4a9eed" strokeWidth="2"/>
      <text x={190} y={65} textAnchor="middle" fill="#4a9eed" fontSize="9" fontWeight="700">Σ wᵢxᵢ</text>
      <text x={190} y={78} textAnchor="middle" fill="#4a9eed" fontSize="8">+ θ</text>
      <line x1={214} y1={70} x2={258} y2={70} stroke="#4a9eed" strokeWidth="1.5" markerEnd="url(#arr-p)"/>
      <rect x={262} y={52} width={60} height={36} rx="6" fill="rgba(74,158,237,0.10)" stroke="#4a9eed" strokeWidth="1.5"/>
      <text x={292} y={68} textAnchor="middle" fill="#4a9eed" fontSize="9" fontWeight="700">f(s)</text>
      <text x={292} y={80} textAnchor="middle" fill="var(--text-secondary)" fontSize="8">activação</text>
      <line x1={322} y1={70} x2={368} y2={70} stroke="#4a9eed" strokeWidth="1.5" markerEnd="url(#arr-p)"/>
      <circle cx={390} cy={70} r={18} fill="rgba(74,158,237,0.10)" stroke="#4a9eed" strokeWidth="2"/>
      <text x={390} y={67} textAnchor="middle" fill="#4a9eed" fontSize="10" fontWeight="700">y</text>
      <text x={390} y={80} textAnchor="middle" fill="#4a9eed" fontSize="8">saída</text>
    </svg>
    <div style={{ display: 'flex', gap: '1.5rem', justifyContent: 'center', alignItems: 'flex-start', marginTop: '0.75rem', flexWrap: 'wrap' }}>
      ...
      <div style={{ background: 'var(--bg-primary)', border: '1px solid rgba(74,158,237,0.10)', borderRadius: 8, padding: '0.6rem 1rem', fontSize: '0.82rem', textAlign: 'left' }}>
        <p style={{ fontWeight: 700, color: '#4a9eed', margin: '0 0 0.35rem' }}>FUNÇÕES f(s)</p>
  ...
  <div style={{ ...S.diagram, textAlign: 'center' }}>
    ...
    <svg viewBox="0 0 620 175" style={{ maxWidth: '100%', height: 'auto' }}>
      ...
      {[70,100,130].flatMap(iy => [60,90,120].map(hy => (
        <line key={`${iy}-${hy}`} x1={42} y1={iy} x2={92} y2={hy} stroke="#4a9eed" strokeWidth="0.8" opacity="0.4"/>
      )))}
      {[60,90,120].map(hy => (
        <line key={hy} x1={108} y1={hy} x2={158} y2={90} stroke="#4a9eed" strokeWidth="0.8" opacity="0.4"/>
      ))}
      {/* nodes on top */}
      {[[30,70],[30,100],[30,130],[100,60],[100,90],[100,120],[170,90]].map(([x,y], i) => (
        <circle key={i} cx={x} cy={y} r={12} fill="rgba(74,158,237,0.10)" stroke="#4a9eed" strokeWidth="1.5"/>
      ))}
      <text x="100" y="158" textAnchor="middle" fill="#4a9eed" fontSize="9">cromossoma = vector de pesos [w₁, w₂, ..., wₙ]</text>
      ...
      {/* gen 0 */}
      <line x1={258} y1={75} x2={308} y2={89} stroke="#4a9eed" strokeWidth="1" opacity="0.5"/>
      <line x1={258} y1={101} x2={308} y2={93} stroke="#4a9eed" strokeWidth="1" opacity="0.5"/>
      {[[248,72],[248,102],[318,92]].map(([nx,ny],j) => (
        <circle key={j} cx={nx} cy={ny} r={10} fill="rgba(74,158,237,0.10)" stroke="#4a9eed" strokeWidth="1.2"/>
      ))}
      <text x={280} y={158} textAnchor="middle" fill="var(--text-secondary)" fontSize="9">gen 0: simples</text>
      {/* gen 10 */}
      <line x1={368} y1={67} x2={418} y2={78} stroke="#4a9eed" strokeWidth="1" opacity="0.5"/>
      <line x1={368} y1={90} x2={418} y2={82} stroke="#4a9eed" strokeWidth="1" opacity="0.5"/>
      <line x1={368} y1={94} x2={418} y2={108} stroke="#4a9eed" strokeWidth="1" opacity="0.5"/>
      <line x1={368} y1={119} x2={418} y2={111} stroke="#4a9eed" strokeWidth="1" opacity="0.5"/>
      ...
      {/* gen 50 */}
      <line x1={478} y1={61} x2={528} y2={67} stroke="#4a9eed" strokeWidth="1" opacity="0.5"/>
      <line x1={478} y1={89} x2={528} y2={95} stroke="#4a9eed" strokeWidth="1" opacity="0.5"/>
      <line x1={478} y1={117} x2={528} y2={123} stroke="#4a9eed" strokeWidth="1" opacity="0.5"/>
      <line x1={477} y1={65} x2={529} y2={91} stroke="#4a9eed" strokeWidth="1" opacity="0.5"/>
...
export default function NEL5() {
  ...
  const approaches = [
    {
      name: 'GA para Pesos', color: '#4a9eed',
    ...
    {
      name: 'PSO / GIL para Pesos', color: '#4a9eed',
    ...
    {
      name: 'Codificação Binária de Topologia', color: '#4a9eed',
            ...
            <table style={S.table}>
              ...
              <tbody>
                {[
                  ...
                  <tr key={w}><td style={{ ...S.td, fontWeight: 600, color: '#4a9eed' }}>{w}</td><td style={S.td}>{r}</td><td style={S.td}>{c}</td><td style={{ ...S.td, color: '#4a9eed' }}>{a}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
          ...
          <div style={S.diagram}>
            ...
            <div style={{ background: 'var(--bg-primary)', borderRadius: 10, padding: '1.25rem', border: `1.5px solid ${a.color}30` }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem', fontSize: '0.85rem' }}>
                ...
                <div><strong style={{ color: '#4a9eed' }}>Fitness:</strong><p style={{ marginTop: '0.35rem', lineHeight: 1.7, color: 'var(--text-primary)' }}>{a.fitness}</p></div>
                <div><strong style={{ color: '#4a9eed' }}>Vantagem:</strong><p style={{ marginTop: '0.35rem', lineHeight: 1.7, color: 'var(--text-primary)' }}>{a.pros}</p></div>
                <div><strong style={{ color: '#4a9eed' }}>Limitação:</strong><p style={{ marginTop: '0.35rem', lineHeight: 1.7, color: 'var(--text-primary)' }}>{a.cons}</p></div>
              </div>
            </div>
          </div>
        </div>
            ...
            <table style={S.table}>
              ...
              <tbody>
                {[
                  ...
                  <tr key={prob}><td style={{ ...S.td, fontWeight: 600, color: 'var(--text-secondary)' }}>{prob}</td><td style={{ ...S.td, color: '#4a9eed' }}>{old}</td><td style={{ ...S.td, color: '#4a9eed' }}>{neat}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
