...
const HillClimbingDiagram = () => (
  <div style={S.diagram}>
    ...
    <svg viewBox="0 0 560 180" style={{ maxWidth: '100%', height: 'auto' }}>
      <defs>
        <marker id="arr-hc" markerWidth="6" markerHeight="6" refX="3" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill="#4a9eed" /></marker>
        <marker id="arr-rr" markerWidth="6" markerHeight="6" refX="3" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill="#4a9eed" /></marker>
      </defs>
      ...
      <polyline points="10,150 60,130 90,100 120,115 150,90 190,60 220,75 250,50 290,30 330,55 360,80 380,60 410,45 450,70 490,100 530,120 550,140 550,170 10,170" fill="rgba(74,158,237,0.10)" stroke="none"/>
      <circle cx={190} cy={60} r={5} fill="#4a9eed" stroke="white" strokeWidth="1.5"/>
      <text x={190} y={48} textAnchor="middle" fill="#4a9eed" fontSize="8" fontWeight="700">ótimo local</text>
      <circle cx={250} cy={50} r={5} fill="#4a9eed" stroke="white" strokeWidth="1.5"/>
      <text x={250} y={38} textAnchor="middle" fill="#4a9eed" fontSize="8" fontWeight="700">ótimo local</text>
      <circle cx={410} cy={45} r={6} fill="#4a9eed" stroke="white" strokeWidth="1.5"/>
      <text x={410} y={28} textAnchor="middle" fill="#4a9eed" fontSize="8" fontWeight="700">ótimo global</text>
      <circle cx={120} cy={115} r={5} fill="#4a9eed" stroke="white" strokeWidth="1.5"/>
      <text x={115} y={130} textAnchor="middle" fill="#4a9eed" fontSize="8">início</text>
      <path d="M 125,113 Q 150,80 185,62" fill="none" stroke="#4a9eed" strokeWidth="1.5" markerEnd="url(#arr-hc)" strokeDasharray="4,2"/>
      <text x={177} y={7} fill="#4a9eed" fontSize="8" fontWeight="700">× preso!</text>
      <circle cx={340} cy={55} r={5} fill="#4a9eed" stroke="white" strokeWidth="1.5"/>
      <text x={340} y={45} textAnchor="middle" fill="#4a9eed" fontSize="8">reinício</text>
      <path d="M 345,53 Q 380,48 405,46" fill="none" stroke="#4a9eed" strokeWidth="1.5" markerEnd="url(#arr-rr)" strokeDasharray="4,2"/>
      <text x={375} y={37} fill="#4a9eed" fontSize="8" fontWeight="700">encontra global ✓</text>
...
const HCVariantExplorer = () => {
  ...
  const variants = [
    {
      name: 'Steepest Ascent', color: '#4a9eed',
    ...
    {
      name: 'First Improvement', color: '#4a9eed',
    ...
    {
      name: 'Random Restart', color: '#4a9eed',
    ...
    {
      name: 'Stochastic HC', color: '#4a9eed',
    ...
    <div style={S.diagram}>
      ...
      <div style={{ background: 'var(--bg-primary)', borderRadius: 10, padding: '1.25rem', textAlign: 'left', border: `1.5px solid ${vr.color}40` }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem', marginBottom: '0.6rem' }}>
          <div><span style={{ fontSize: '0.72rem', color: '#4a9eed', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Vantagens</span>{vr.pros.map(p => <div key={p} style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', marginTop: '0.15rem' }}>✓ {p}</div>)}</div>
          <div><span style={{ fontSize: '0.72rem', color: '#4a9eed', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Limitações</span>{vr.cons.map(c => <div key={c} style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', marginTop: '0.15rem' }}>✗ {c}</div>)}</div>
        </div>
...
const SADiagram = () => (
  <div style={S.diagram}>
    ...
    <svg viewBox="0 0 560 175" style={{ maxWidth: '100%', height: 'auto' }}>
      <defs>
        <marker id="arr-sa" markerWidth="6" markerHeight="6" refX="3" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill="#4a9eed" /></marker>
      </defs>
      ...
      <polyline points="20,140 60,110 90,80 120,95 155,65 190,85 220,50 260,25 300,50 330,80 360,55 400,35 440,65 480,100 520,130 550,145 550,165 20,165" fill="rgba(74,158,237,0.10)" stroke="none"/>
      <circle cx={90} cy={80} r={5} fill="#4a9eed" stroke="white" strokeWidth="1.5"/>
      <text x={75} y={72} fill="#4a9eed" fontSize="8" fontWeight="700">início</text>
      <path d="M 95,78 Q 120,75 150,67" fill="none" stroke="#4a9eed" strokeWidth="1.5" markerEnd="url(#arr-sa)"/>
      <circle cx={155} cy={65} r={5} fill="#4a9eed" stroke="white" strokeWidth="1.5"/>
      <text x={155} y={53} textAnchor="middle" fill="#4a9eed" fontSize="8">ótimo local</text>
      <path d="M 160,67 Q 175,85 186,83" fill="none" stroke="#4a9eed" strokeWidth="1.5" strokeDasharray="3,2" markerEnd="url(#arr-sa)"/>
      <text x={175} y={100} textAnchor="middle" fill="#4a9eed" fontSize="8">piora aceite!</text>
      <text x={175} y={110} textAnchor="middle" fill="#4a9eed" fontSize="7">(prob. e^-Δf/T)</text>
      <path d="M 195,82 Q 225,55 255,27" fill="none" stroke="#4a9eed" strokeWidth="1.5" markerEnd="url(#arr-sa)"/>
      <circle cx={260} cy={25} r={6} fill="#4a9eed" stroke="white" strokeWidth="1.5"/>
      <text x={260} y={13} textAnchor="middle" fill="#4a9eed" fontSize="8" fontWeight="700">ótimo global!</text>
      <rect x="20" y="152" width="530" height="8" rx="4" fill="url(#tempGrad)"/>
      <defs>
        <linearGradient id="tempGrad" x1="0%" y1="0%" x2="100%" y2="0%">
          <stop offset="0%" stopColor="#4a9eed" stopOpacity="0.9"/>
          <stop offset="100%" stopColor="#4a9eed" stopOpacity="0.15"/>
        </linearGradient>
      </defs>
      <text x="20" y="170" fill="#4a9eed" fontSize="8">T alto — aceita muitas pioras</text>
      ...
      <div style={S.page}>
        ...
        <div style={S.section}>
          ...
          <div style={{ background: 'var(--bg-secondary)', border: '1px solid var(--card-border)', borderRadius: 8, padding: '1rem', marginBottom: '1.5rem', fontSize: '0.88rem', lineHeight: 2, color: 'var(--text-primary)' }}>
            <div style={{ fontWeight: 700, marginBottom: '0.5rem', color: '#4a9eed' }}>Hill Climbing (maximização)</div>
            ...
            <table style={S.table}>
              ...
              <tbody>
                {[
                  ...
                  <tr key={p}><td style={{ ...S.td, fontWeight: 600, color: '#4a9eed' }}>{p}</td><td style={S.td}>{d}</td><td style={{ ...S.td, fontSize: '0.83rem', color: 'var(--text-secondary)' }}>{q}</td><td style={{ ...S.td, color: '#4a9eed', fontSize: '0.83rem' }}>{m}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
        ...
        <div style={S.section}>
          ...
          <div style={{ background: 'var(--bg-secondary)', border: '1px solid rgba(74,158,237,0.10)', borderRadius: 8, padding: '1rem', textAlign: 'center', margin: '1rem 0' }}>
            <div style={{ fontSize: '1.2rem', color: '#4a9eed', fontWeight: 700 }}>
              <BlockMath math="P(\text{aceitar}) = e^{-|\Delta f| / T}" />
            </div>
          ...
          <div style={{ overflowX: 'auto', marginBottom: '1.5rem' }}>
            <table style={S.table}>
              ...
              <tbody>
                {[
                  ['T = 100', 'e^{-0.1} \\approx 0.905', 'Aceita quase tudo — exploração ampla, quase aleatório'],
                  ['T = 10',  'e^{-1} \\approx 0.368',   'Aceita ~37% das pioras — equilíbrio exploração/refinamento'],
                  ['T = 1',   'e^{-10} \\approx 0.000045', 'Raramente aceita pioras — comportamento próximo de Hill Climbing'],
                  ['T = 0.1', 'e^{-100} \\approx 0',      'Nunca aceita pioras — idêntico a Hill Climbing'],
                ].map(([t, p, b]) => (
                  <tr key={t}><td style={{ ...S.td, fontFamily: 'monospace', color: '#4a9eed', fontWeight: 600 }}>{t}</td><td style={{ ...S.td, fontSize: '0.83rem', textAlign: 'center' }}><InlineMath math={p} /></td><td style={S.td}>{b}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
        ...
        <div style={S.section}>
          ...
          <div style={S.diagram}>
            ...
            <div style={{ background: 'var(--bg-primary)', borderRadius: 10, padding: '1.25rem', textAlign: 'left', border: `1.5px solid ${c.color}40` }}>
              <div style={{ background: `${c.color}10`, border: `1px solid ${c.color}25`, borderRadius: 6, padding: '0.5rem 0.75rem', marginBottom: '0.75rem', fontSize: '0.95rem', color: c.color, textAlign: 'center' }}>
                {c.formula ? <InlineMath math={c.formula} /> : 'T ajustado com base na taxa de aceitação de pioras'}
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem', fontSize: '0.83rem' }}>
                <div><strong style={{ color: '#4a9eed' }}>Vantagem:</strong> <span style={{ color: 'var(--text-secondary)' }}>{c.pros}</span></div>
                <div><strong style={{ color: '#4a9eed' }}>Limitação:</strong> <span style={{ color: 'var(--text-secondary)' }}>{c.cons}</span></div>
              </div>
            </div>
          </div>

          <h3 style={S.h3}>Convergência do SA</h3>
          <p style={S.p}>O Teorema de Convergência Assimptótica do SA garante que o algoritmo converge para o ótimo global com probabilidade 1 — mas apenas sob duas condições: o cooling schedule logarítmico (<InlineMath math="T(t) = c/\log(1+t)" />, impraticavelmente lento) e a vizinhança deve ser completamente conexa (qualquer solução acessível a partir de qualquer outra por uma sequência de passos unitários).</p>
