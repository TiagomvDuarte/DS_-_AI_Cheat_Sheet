...
function ArchSVG() {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', display: 'block' }}>
      ...
      <text x={padL + innerW / 2} y={H - 4} fontSize="9" fill="var(--text-secondary)" textAnchor="middle">ARCH(1): omega=0.1, alpha=0.7 — sigma^2 dispara apos grandes erros</text>
    </svg>
  );
}
...
function NewsImpactSVG() {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', display: 'block' }}>
      ...
      <text x={padL + innerW - 10} y={padT + 14} fontSize="9" fill={color} textAnchor="end">GARCH (simetrico)</text>
      <text x={padL + innerW - 10} y={padT + 28} fontSize="9" fill={color} textAnchor="end">GJR-GARCH (assimetrico)</text>
...
function VaRSVG() {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', display: 'block' }}>
      ...
      <text x={padL + innerW / 2} y={padT + innerH + 32} fontSize="9" fill="var(--text-secondary)" textAnchor="middle">Distribuicao dos retornos: area sombreada = VaR a 95%</text>
...
export default function ST15() {
  return (
    <div style={S.page}>
      ...
      <div style={S.tag}>MÓDULO 15</div>
      <h1 style={S.h1}>Modelos de Volatilidade</h1>
      <p style={S.lead}>
        A volatilidade dos activos financeiros nao e constante ao longo do tempo — agrupa-se em periodos
        de turbulencia e de acalmia. Os modelos ARCH e GARCH capturam este comportamento, tornando-se
        ferramentas essenciais para gestao de risco, apreçamento de derivados e calculo do VaR.
      </p>

      {/* ── 1. Cluster de Volatilidade ── */}
      <section style={S.section}>
        ...
        <p style={S.p}>
          ...
          do activo ou mercado considerado. Compreender estes padroes e o ponto de partida para a modelacao
          da volatilidade.
        </p>
        ...
        <div style={S.highlight}>
          ...
          <ul style={{ marginTop: '0.5rem', paddingLeft: '1.4rem', lineHeight: 2 }}>
            <li><strong>Caudas pesadas (fat tails):</strong> a distribuicao dos retornos tem mais massa nas caudas do que a Normal — excesso de curtose positivo.</li>
            ...
            <li><strong>Clustering de volatilidade:</strong> grandes variacoes tendem a ser seguidas de grandes variacoes, e periodos calmos tendem a ser calmos — autocorrelacao nos |rt| e rt².</li>
          </ul>
        </div>
        ...
        <p style={S.p}>
          ...
          foram desenhados precisamente para capturar esta dependencia temporal na variancia condicional.
        </p>

        <div style={S.note}>
          A autocorrelacao dos retornos rt e tipicamente nula (hipotese de eficiencia), mas a autocorrelacao
      ...
      <section style={S.section}>
        ...
        <p style={S.p}>
          ...
          (ARCH), pelo qual recebeu o Premio Nobel em 2003. A ideia central e modelar a variancia condicional
          como funcao dos quadrados dos erros passados.
        </p>

        <h3 style={S.h3}>Especificacao</h3>
        ...
        <p style={S.p}>
          O processo zt e ruido branco estandardizado (Normal(0,1) ou t-Student). A variancia condicional
          sigma_t² reage imediatamente a choques passados.
        </p>

        <h3 style={S.h3}>Condicao de Estacionaridade</h3>
        <p style={S.p}>
          Para que o processo seja estacionario de segunda ordem, a soma dos coeficientes auto-regressivos
        ...
        <h3 style={S.h3}>Exemplo Numerico — ARCH(1): omega=0.1, alpha=0.7</h3>
        <p style={S.p}>
          Simulamos 6 periodos com erros epsilon_t = {archEps.join(', ')}. A variancia condicional
          dispara nos periodos t=3 e t=4 apos os choques de 1.8 e -2.1.
        </p>
        ...
        <table style={S.table}>
          ...
          <tbody>
            {archEps.map((e, i) => (
              <tr key={i} style={archSigma2[i] > 1.5 ? { background: 'rgba(249,115,22,0.10)' } : {}}>
        ...
        <div style={S.note}>
          Limitacao do ARCH(q): para capturar persistencia de volatilidade prolongada, sao necessarios
          valores de q muito elevados, tornando a estimacao instavel. O GARCH resolve este problema
          com apenas um parametro adicional beta.
        </div>
      </section>
      ...
      <section style={S.section}>
        ...
        <p style={S.p}>
          Antes de ajustar um modelo GARCH, e boa pratica verificar se existe heteroscedasticidade
          condicional nos residuos. O teste LM de Engle e o procedimento padrao.
        </p>

        <h3 style={S.h3}>Procedimento</h3>
        <ol style={{ paddingLeft: '1.4rem', lineHeight: 2, color: 'var(--text-primary)', fontSize: '1rem' }}>
          <li>Estimar o modelo de media (AR, ARMA, etc.) e obter os residuos e_t.</li>
          <li>Calcular e_t² e regredir sobre e_(t-1)², ..., e_(t-q)².</li>
          <li>Calcular a estatistica LM = n · R², onde n e o numero de observacoes.</li>
        ...
        <p style={S.p}>
          Um p-valor baixo (p &lt; 0.05) indica a presenca de efeitos ARCH — a variancia condicional
          nao e constante e um modelo GARCH e justificado.
        </p>

        <div style={S.note}>
          Em amostras grandes (tipicamente n &gt; 1000 em dados intradias), o teste ARCH quase sempre
          rejeita H0. O interesse pratico esta na magnitude e persistencia dos efeitos, nao apenas
          na sua presenca.
        </div>
      </section>
      ...
      <section style={S.section}>
        ...
        <p style={S.p}>
          Tim Bollerslev generalizou o ARCH introduzindo a variancia condicional desfasada no lado
          direito da equacao. O <strong>GARCH(1,1)</strong> e de longe o modelo mais utilizado em finanças,
          capturando a persistencia da volatilidade com apenas tres parametros.
        </p>
        ...
        <p style={S.p}>
          O parametro alpha (news coefficient) mede a reaccao imediata a novos choques; beta (persistence)
          mede a velocidade de reversao a variancia de longo prazo. Quando alpha + beta e proximo de 1,
          a volatilidade e altamente persistente — choques demoram muito a dissipar-se.
        </p>

        <h3 style={S.h3}>Condicoes de Estacionaridade e Positividade</h3>
        <table style={S.table}>
          <thead>
            <tr>
              <th style={S.th}>Condicao</th>
          ...
          <tbody>
            {[
              ['Positividade de sigma_t²', 'omega > 0, alpha >= 0, beta >= 0', 'Variancia sempre positiva'],
              ['Estacionaridade', 'alpha + beta < 1', 'Variancia incondicional finita'],
              ['Processo IGARCH', 'alpha + beta = 1', 'Choque permanente (sem reversao)'],
        ...
        <p style={S.p}>
          ...
          estimado via recursao GARCH(1,1) com omega=0.05, alpha=0.10, beta=0.85. Observa-se claramente
      ...
      {/* ── 5. Estimacao GARCH ── */}
      <section style={S.section}>
        <h2 style={S.h2}>5. Estimacao por Maxima Verosimilhança</h2>
        <p style={S.p}>
          Os parametros GARCH sao estimados maximizando a log-verosimilhanca condicional. A funcao
          de verosimilhanca depende da distribuicao assumida para as inovacoes z_t.
        </p>
        ...
        <p style={S.p}>
          Na pratica, as inovacoes financeiras tem caudas mais pesadas do que a Normal. Distribuicoes
          alternativas melhoram o ajustamento e produzem estimativas mais robustas do VaR.
        </p>

        <h3 style={S.h3}>Distribuicoes das Inovacoes</h3>
        <table style={S.table}>
          <thead>
            <tr>
              <th style={S.th}>Distribuicao</th>
              <th style={S.th}>Parametros extras</th>
          ...
          <tbody>
            {[
              ['Normal', '—', 'Simples, QML consistente', 'Estimacao inicial'],
              ['t-Student', 'graus de liberdade nu', 'Caudas pesadas', 'VaR, stress testing'],
              ['t assimetrica (skewed-t)', 'nu, skewness xi', 'Caudas + assimetria', 'Accoes individuais'],
        ...
        <h3 style={S.h3}>Comparacao de Modelos via AIC/BIC</h3>
        <table style={S.table}>
          <thead>
            <tr>
              <th style={S.th}>Modelo</th>
              <th style={S.th}>Parametros</th>
          ...
          <tbody>
            {[
              ...
              <tr key={i} style={row.best ? { background: 'rgba(249,115,22,0.10)' } : {}}>
        ...
        <div style={S.note}>
          O EGARCH(1,1) com inovacoes t-Student (*) apresenta melhor AIC e BIC neste exemplo. Na
          pratica, o GARCH(1,1) simples e frequentemente competitivo (principio da parcimonia de
          Hansen e Lunde, 2005).
        </div>
      </section>
      ...
      {/* ── 6. Leverage e GARCH Assimetrico ── */}
      <section style={S.section}>
        <h2 style={S.h2}>6. Efeito de Alavancagem e Modelos Assimetricos</h2>
        <p style={S.p}>
          Um facto empirico bem documentado em mercados de accoes e que retornos negativos (mas noticias)
        ...
        <p style={S.p}>
          A formulacao logaritmica do EGARCH garante sigma_t² &gt; 0 sem necessidade de impor restricoes
          de sinal sobre os parametros. O coeficiente gamma captura directamente o efeito de alavancagem.
        </p>

        <h3 style={S.h3}>Curva de Impacto de Noticias (News Impact Curve)</h3>
        <p style={S.p}>
          A NIC representa sigma_t² como funcao de epsilon_(t-1), mantendo sigma_(t-1)² no seu valor incondicional.
          Para o GARCH simetrico e uma parabola; para o GJR-GARCH e assimetrica com o ramo esquerdo mais inclinado.
        </p>
        ...
        <div style={S.note}>
          A linha continua (GARCH simetrico) tem a mesma inclinacao para choques positivos e negativos.
      ...
      {/* ── 7. Previsao e VaR ── */}
      <section style={S.section}>
        <h2 style={S.h2}>7. Previsao de Volatilidade e Value at Risk</h2>
        <p style={S.p}>
          Uma das principais utilizacoes dos modelos GARCH e produzir previsoes de volatilidade e,
          a partir destas, calcular medidas de risco como o Value at Risk (VaR).
        </p>

        <h3 style={S.h3}>Previsao h-Step GARCH(1,1)</h3>
        ...
        <p style={S.p}>
          Para horizontes longos, a previsao converge para a variancia incondicional a uma taxa determinada
          por alpha+beta. Quando alpha+beta ≈ 0.97, a convergencia e muito lenta — a volatilidade
          presente continua a ser informativa mesmo para previsoes a 30 dias.
        </p>
        ...
        <p style={S.p}>
          Ao usar sigma_t dinamico em vez de sigma constante, o VaR-GARCH e mais conservador em periodos
          volateis e mais permissivo em periodos calmos — adaptando-se as condicoes de mercado.
        </p>
        ...
        <p style={S.p}>
          ...
          Com inovacoes t-Student, esta area e ligeiramente mais alargada, produzindo um VaR mais conservador.
        </p>
        ...
        <table style={S.table}>
          ...
          <tbody>
            {[
              ['Kupiec (1995)', 'Frequencia de violacoes = alpha', 'Proportion of Failures (POF)'],
              ['Christoffersen (1998)', 'Violacoes independentes e iid', 'Coverage + independence'],
              ['DQ Test (Engle & Manganelli)', 'Hit sequence sem autocorrelacao', 'Dynamic Quantile Test'],
            ].map(([t, h, r], i) => (
              <tr key={i}>
                <td style={S.td}>{t}</td>
                <td style={S.td}>{h}</td>
      ...
      {/* ── 8. DCC-GARCH ── */}
      <section style={S.section}>
        <h2 style={S.h2}>8. DCC-GARCH — Correlacoes Dinamicas</h2>
        <p style={S.p}>
          O modelo <strong>Dynamic Conditional Correlation GARCH</strong> (DCC), proposto por Engle (2002),
          estende o GARCH multivariado permitindo que as correlacoes entre activos variem ao longo do tempo.
        </p>
        ...
        <p style={S.p}>
          A estimacao e feita em dois passos: primeiro estimam-se os GARCH univariados para cada serie;
          depois estimam-se os parametros de correlacao dinamica (a, b) a partir dos residuos
          estandardizados. Esta abordagem torna o DCC escalavel para carteiras com muitos activos.
        </p>

        <h3 style={S.h3}>Aplicacoes do DCC-GARCH</h3>
        <ul style={{ paddingLeft: '1.4rem', lineHeight: 2, color: 'var(--text-primary)', fontSize: '1rem' }}>
          <li><strong>Risco de carteira:</strong> calcular VaR multivariado com correlacoes variaveis.</li>
          <li><strong>Cobertura dinamica (hedging):</strong> racios de cobertura que se ajustam com as correlacoes.</li>
          <li><strong>Deteccao de contagio:</strong> correlacoes que sobem durante crises financeiras.</li>
          <li><strong>Gestao de activos:</strong> diversificacao efectiva em funcao das correlacoes correntes.</li>
        </ul>

        <div style={S.note}>
          Durante crises (2008, COVID-19), as correlacoes entre activos de risco tendem a convergir
          para 1 — o chamado efeito "correlation breakdown" que reduz os beneficios da diversificacao
          exactamente quando mais sao necessarios. O DCC captura este fenomeno.
        </div>
      </section>

      <hr style={S.divider} />

      {/* ── 9. Volatilidade Realizada ── */}
      <section style={S.section}>
        <h2 style={S.h2}>9. Volatilidade Realizada e HAR-RV</h2>
        <p style={S.p}>
          Com dados de alta frequencia (tick-by-tick, 1 minuto, 5 minutos), e possivel estimar a
        ...
        <p style={S.p}>
          Quando o numero de observacoes intradias cresce, RV_t converge para a variancia quadratica
          integrada (variancia "verdadeira" do dia), tornando-se uma estimativa quase sem erro de
          medicao. Contudo, microestrutura (bid-ask bounce, asynchronous trading) pode introduzir
          ruido que exige correccoes.
        </p>
        ...
        <p style={S.p}>
          O HAR-RV e um modelo de heterogeneidade ao nivel do agente: operadores de curto prazo
          ...
          modelos GARCH em previsao out-of-sample.
        </p>

        <h3 style={S.h3}>Comparacao: GARCH vs. Volatilidade Realizada</h3>
        <table style={S.table}>
          <thead>
            <tr>
              <th style={S.th}>Dimensao</th>
          ...
          <tbody>
            {[
              ['Dados necessarios', 'Retornos diarios', 'Dados intradias (HF)'],
              ['Tipo de modelo', 'Parametrico', 'Semi-parametrico / nao-param.'],
              ['Erro de medicao', 'sigma_t e latente', 'RV_t ≈ variancia verdadeira'],
              ['Previsao curto prazo', 'Razoavel', 'Superior (HAR-RV)'],
              ['Previsao longo prazo', 'Mean-reversion lenta', 'HAR-RV em escala RV'],
              ['Complexidade', 'Baixa–media', 'Requer infra HF'],
              ['Uso em VaR', 'Padrao regulatorio', 'Crescente adopcao'],
            ].map(([d, g, rv], i) => (
              <tr key={i}>
      ...
      <section style={S.section}>
        ...
        <p style={S.p}>
          Os modelos de volatilidade sao fundamentais para qualquer aplicacao em financas quantitativas
          que envolva incerteza temporal. A evolucao — de ARCH a GARCH, EGARCH, GJR-GARCH, DCC e
          Volatilidade Realizada — reflecte decadas de investigacao para capturar com crescente precisao
          os padroes da volatilidade financeira.
        </p>
        ...
        <table style={S.table}>
          <thead>
            <tr>
              <th style={S.th}>Modelo</th>
              <th style={S.th}>Inovacao principal</th>
              <th style={S.th}>Quando usar</th>
            </tr>
          </thead>
          <tbody>
            {[
              ['ARCH(q)', 'Variancia condicional auto-regressiva', 'Introducao, series curtas'],
              ['GARCH(1,1)', 'Persistencia com poucos parametros', 'Ponto de partida universal'],
              ['GJR-GARCH', 'Efeito de alavancagem via dummy', 'Accoes, indices de equity'],
              ['EGARCH', 'Log-variancia + assimetria directa', 'Quando sigma² > 0 sem restricoes'],
              ['DCC-GARCH', 'Correlacoes dinamicas multivariadas', 'Carteiras, hedging'],
              ['HAR-RV', 'Dados HF + heterogeneidade temporal', 'Alta frequencia, previsao CP'],
            ].map(([m, inn, u], idx) => (
              <tr key={idx}>
                <td style={{ ...S.td, fontWeight: 700, color }}>{m}</td>
                <td style={S.td}>{inn}</td>
                <td style={S.td}>{u}</td>
              </tr>
            ))}
          </tbody>
        ...
        <div style={S.highlight}>
          <strong style={{ color }}>Principios orientadores:</strong>
          <ul style={{ marginTop: '0.5rem', paddingLeft: '1.4rem', lineHeight: 2 }}>
            <li>Comecar sempre pelo GARCH(1,1) — e o benchmark. Modelos mais complexos so se justificam com melhoria clara de AIC/BIC e diagnosticos.</li>
            <li>Verificar efeitos ARCH antes de ajustar (ArchTest). Verificar ausencia de efeitos ARCH residuais apos ajuste.</li>
            <li>Usar distribuicao t-Student ou skewed-t — a Normal subestima o risco nas caudas.</li>
            <li>Fazer backtesting do VaR (Kupiec, Christoffersen) para validar a adequacao do modelo para fins de risco.</li>
            <li>Em dados diarios de equity, testar sempre o efeito de alavancagem (GJR ou EGARCH).</li>
          </ul>
        </div>
