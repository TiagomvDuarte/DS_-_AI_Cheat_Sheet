...
const CAPDiagram = () => (
  <div style={S.diagram}>
    ...
    <svg viewBox="0 0 400 240" style={{ maxWidth: '100%', height: 'auto' }}>
      ...
      <circle cx="200" cy="35" r="30" fill="var(--bg-secondary)" stroke="#f97316" strokeWidth="2" />
      <text x="200" y="30" textAnchor="middle" fill="#f97316" fontSize="11" fontWeight="800">C</text>
      <text x="200" y="44" textAnchor="middle" fill="#f97316" fontSize="9">Consistency</text>
      <circle cx="40" cy="200" r="30" fill="var(--bg-secondary)" stroke="#f97316" strokeWidth="2" />
      <text x="40" y="195" textAnchor="middle" fill="#f97316" fontSize="11" fontWeight="800">A</text>
      <text x="40" y="209" textAnchor="middle" fill="#f97316" fontSize="9">Availability</text>
      <circle cx="360" cy="200" r="30" fill="var(--bg-secondary)" stroke="#f97316" strokeWidth="2" />
      <text x="360" y="195" textAnchor="middle" fill="#f97316" fontSize="11" fontWeight="800">P</text>
      <text x="360" y="209" textAnchor="middle" fill="#f97316" fontSize="9">Partition Tol.</text>
...
const NoSQLExplorer = () => {
  ...
  const dbs = [
    {
      name: 'Redis', family: 'Key-Value', color: '#f97316', cap: 'AP (por omissão)',
    ...
    {
      name: 'MongoDB', family: 'Document', color: '#f97316', cap: 'CP (com Replica Sets)',
    ...
    {
      name: 'Cassandra', family: 'Wide-Column', color: '#f97316', cap: 'AP (tunable consistency)',
    ...
    {
      name: 'Neo4j', family: 'Graph', color: '#f97316', cap: 'CA (single node) / CP (cluster)',
  ...
  return (
    <div style={S.diagram}>
      ...
      <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'center', marginBottom: '1.25rem', flexWrap: 'wrap' }}>
        {dbs.map((dbItem, i) => (
          <button key={i} onClick={() => setDb(i)} style={{ padding: '0.4rem 0.9rem', borderRadius: 20, cursor: 'pointer', fontWeight: 600, fontSize: '0.82rem', background: db === i ? dbItem.color : 'var(--bg-primary)', color: db === i ? (dbItem.color === '#f97316' ? '#000' : 'white') : 'var(--text-primary)', border: `1.5px solid ${db === i ? dbItem.color : 'var(--card-border)'}`, transition: 'all 0.2s' }}>{dbItem.name} <span style={{ fontSize: '0.72rem', opacity: 0.85 }}>({dbItem.family})</span></button>
        ))}
      </div>
      <div style={{ background: 'var(--bg-primary)', borderRadius: 10, padding: '1.25rem', textAlign: 'left', border: `1.5px solid ${d.color}40` }}>
        <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap', marginBottom: '0.75rem' }}>
          ...
          <span style={{ background: 'rgba(249,115,22,0.10)', border: '1px solid rgba(249,115,22,0.10)', color: '#f97316', fontSize: '0.75rem', fontWeight: 600, padding: '0.15rem 0.6rem', borderRadius: 12 }}>{d.usecase}</span>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '0.75rem' }}>
          <div>
            <div style={{ fontSize: '0.72rem', color: '#f97316', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.3rem' }}>Vantagens</div>
            {d.pros.map(p => <div key={p} style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', marginBottom: '0.15rem' }}>✓ {p}</div>)}
          </div>
          <div>
            <div style={{ fontSize: '0.72rem', color: '#f97316', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.3rem' }}>Limitações</div>
  ...
  return (
    <div style={{ padding: '2rem 1rem' }}>
      <div style={S.page}>
        ...
        <p style={S.lead}>Da evolução histórica dos sistemas de gestão de dados ao Teorema CAP. Porque o RDBMS não escala horizontalmente, e como as 4 famílias NoSQL com arquitectura interna, trade-offs e casos de uso reais resolvem esses problemas. Bases especializadas emergentes e o padrão de polyglot persistence em produção.</p>
        ...
        <div style={S.section}>
          ...
          <div style={S.highlight}>
            <p style={{ fontWeight: 700, color: '#f97316', marginBottom: '0.5rem' }}>Propriedades essenciais de uma Base de Dados</p>
        ...
        <div style={S.section}>
            ...
            <table style={S.table}>
              ...
              <tbody>
                {[
                  ...
                  <tr key={f}><td style={{ ...S.td, fontWeight: 600 }}>{f}</td><td style={S.td}>{r}</td><td style={{ ...S.td, color: '#f97316' }}>{n}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
            <div style={{ background: 'rgba(249,115,22,0.06)', border: '1px solid rgba(249,115,22,0.2)', borderRadius: 8, padding: '1rem' }}>
              <div style={{ fontWeight: 700, color: '#f97316', marginBottom: '0.5rem' }}>ACID (RDBMS)</div>
              ...
              <div style={{ fontSize: '0.82rem', color: '#f97316', marginTop: '0.5rem' }}>✓ Banca, pagamentos, sistemas críticos, inventário</div>
            </div>
            <div style={{ background: 'rgba(249,115,22,0.10)', border: '1px solid rgba(249,115,22,0.10)', borderRadius: 8, padding: '1rem' }}>
              <div style={{ fontWeight: 700, color: '#f97316', marginBottom: '0.5rem' }}>BASE (NoSQL)</div>
              <ul style={{ margin: 0, paddingLeft: '1.2rem', fontSize: '0.84rem', color: 'var(--text-secondary)', lineHeight: 1.9 }}>
              ...
              <div style={{ fontSize: '0.82rem', color: '#f97316', marginTop: '0.5rem' }}>✓ Redes sociais, IoT, analytics, caching, profiles</div>
            </div>
          </div>
        </div>
        ...
        <div style={S.section}>
          ...
          <div style={{ background: 'var(--bg-secondary)', borderRadius: 10, padding: '1.25rem', fontSize: '0.87rem', lineHeight: 2, marginBottom: '1.5rem' }}>
            ...
            <div>• <span style={{ color: '#f97316', fontWeight: 600 }}>Apache Cassandra</span> — histórico de visualizações de 200M+ utilizadores (escrita massiva, alta disponibilidade, multi-region)</div>
            <div>• <span style={{ color: '#f97316', fontWeight: 600 }}>Amazon S3 + Parquet</span> — dados brutos para analytics batch com Spark</div>
            <div>• <span style={{ color: '#f97316', fontWeight: 600 }}>Elasticsearch</span> — pesquisa de títulos e catálogo de 15.000+ títulos</div>
            <div>• <span style={{ color: '#f97316', fontWeight: 600 }}>Redis</span> — cache de recomendações personalizadas e sessões ativas</div>
            <div>• <span style={{ color: '#f97316', fontWeight: 600 }}>MySQL (Aurora)</span> — dados de billing, subscrições e contratos (ACID obrigatório)</div>
            <div>• <span style={{ color: '#f97316', fontWeight: 600 }}>InfluxDB</span> — métricas de performance dos servidores e CDN</div>
          </div>
            ...
            <table style={S.table}>
              ...
              <tbody>
                {[
                  ...
                  <tr key={q}><td style={S.td}>{q}</td><td style={{ ...S.td, color: '#f97316', fontWeight: 600, fontSize: '0.85rem' }}>{r}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
        ...
        <div style={S.section}>
          ...
          <div style={S.highlight}>
            <ul style={{ paddingLeft: '1.2rem', margin: 0, lineHeight: 1.9 }}>
              <li style={{ marginBottom: '0.4rem' }}><strong>CAP Theorem</strong> — em sistemas distribuídos, Partition Tolerance é obrigatório; a escolha real é CP (consistência) vs. AP (disponibilidade) quando ocorre uma partição de rede.</li>
              <li style={{ marginBottom: '0.4rem' }}><strong>As 4 famílias NoSQL</strong> — Redis (key-value, sub-ms in-memory), MongoDB (document, schema flexível), Cassandra (wide-column, escrita massiva peer-to-peer), Neo4j (graph, traversal O(1) por hop).</li>
              <li style={{ marginBottom: '0.4rem' }}><strong>Polyglot persistence</strong> — nenhuma base de dados é óptima para tudo; sistemas de produção combinam múltiplas BDs especializadas (ex: Netflix usa Cassandra + Redis + Elasticsearch + MySQL em simultâneo).</li>
              <li style={{ marginBottom: '0.4rem' }}><strong>Delta Lake / Iceberg</strong> — resolvem o problema histórico dos Data Lakes adicionando ACID + schema evolution + time travel sobre ficheiros Parquet em object storage.</li>
              <li style={{ marginBottom: '0.4rem' }}><strong>Vector DBs (Pinecone, Weaviate)</strong> — infra-estrutura central do boom de RAG com LLMs; permitem queries de similaridade semântica (ANN) em vez de exact match.</li>
            </ul>
          </div>
        </div>
      </div>
