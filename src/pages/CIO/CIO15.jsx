...
const MembershipDiagram = () => (
  <div style={{ ...S.diagram, textAlign: 'center' }}>
    ...
    <svg viewBox="0 0 560 160" style={{ maxWidth: '100%', height: 'auto' }}>
      ...
      <path d="M 30,30 L 130,30 L 230,130" fill="rgba(59,130,246,0.15)" stroke="#4a9eed" strokeWidth="2"/>
      <text x={100} y={55} fill="#4a9eed" fontSize="9" fontWeight="700">Frio</text>

      {/* Tepid membership (green triangle) */}
      <path d="M 130,130 L 255,30 L 380,130" fill="rgba(74,158,237,0.10)" stroke="#4a9eed" strokeWidth="2"/>
      <text x={255} y={25} textAnchor="middle" fill="#4a9eed" fontSize="9" fontWeight="700">Morno</text>

      {/* Hot membership (red trapezoid) */}
      <path d="M 305,130 L 405,30 L 505,30" fill="rgba(74,158,237,0.10)" stroke="#4a9eed" strokeWidth="2"/>
      <text x={400} y={55} fill="#4a9eed" fontSize="9" fontWeight="700">Quente</text>

      {/* Example: T=30°C */}
      <line x1={205} y1={130} x2={205} y2={20} stroke="#4a9eed" strokeWidth="1.5" strokeDasharray="4,3"/>
      <circle cx={205} cy={57} r={4} fill="#4a9eed"/>
      <circle cx={205} cy={105} r={4} fill="#4a9eed"/>
      <text x={160} y={45} fill="#4a9eed" fontSize="8.5" fontWeight="700">T=30°C</text>
      <text x={160} y={55} fill="#4a9eed" fontSize="7.5">μ_frio=0.5</text>
      <text x={215} y={95} fill="#4a9eed" fontSize="7.5">μ_morno=0.5</text>
  ...
  const steps = [
    {
      name: 'Fuzzificação',
      color: '#4a9eed',
      desc: <>Converter inputs numéricos (crisp) em graus de pertença a conjuntos fuzzy. Para cada variável de entrada, calcula-se <InlineMath math="\mu_A(x)" /> para cada conjunto fuzzy A (e.g., Frio, Morno, Quente). O resultado é um vector de graus de pertença.</>,
      exemplo: <>Temperatura = 30°C → <InlineMath math="\mu_{frio}(30)=0.5" />, <InlineMath math="\mu_{morno}(30)=0.5" />, <InlineMath math="\mu_{quente}(30)=0.0" /></>,
    },
    {
      name: 'Inferência',
      color: '#4a9eed',
    ...
    },
    {
      name: 'Defuzzificação',
      color: '#4a9eed',
      desc: <>Converter o conjunto fuzzy agregado de output num valor numérico (crisp). O método mais comum é o centróide (centro de gravidade): <InlineMath math="\dfrac{\int x \cdot \mu(x)\,dx}{\int \mu(x)\,dx}" />. Alternativas: bisector, MOM (mean of maxima), SOM, LOM.</>,
    ...
    <div style={{ padding: '2rem 1rem' }}>
      <div style={S.page}>
        ...
        <div style={S.section}>
          ...
          <p style={S.p}>Um conjunto fuzzy A sobre um universo de discurso X é definido por uma função de pertença <InlineMath math="\mu_A: X \to [0,1]" />. <InlineMath math="\mu_A(x) = 1" /> significa pertença total; <InlineMath math="\mu_A(x) = 0" /> significa não-pertença; qualquer valor intermédio representa pertença parcial. Um elemento pode pertencer simultaneamente a múltiplos conjuntos fuzzy com graus diferentes.</p>
          <p style={S.p}>As formas mais comuns de funções de pertença são: triangular (3 parâmetros), trapezoidal (4 parâmetros), gaussiana (2 parâmetros: <InlineMath math="\mu, \sigma" />), e sigmoidal. A escolha depende do domínio e do conhecimento especialista.</p>
          <MembershipDiagram />
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={S.table}>
              ...
              <tbody>
                {[
                  ['Intersecção (AND)', 'A \\cap B = \\min(A,B) = \\{0,1\\}', '\\mu_{A \\cap B}(x) = \\min(\\mu_A(x), \\mu_B(x))', '\\mu_{A \\cap B}(x) = \\mu_A(x) \\cdot \\mu_B(x)'],
                  ['União (OR)', 'A \\cup B = \\max(A,B) = \\{0,1\\}', '\\mu_{A \\cup B}(x) = \\max(\\mu_A(x), \\mu_B(x))', '\\mu_{A \\cup B}(x) = \\mu_A + \\mu_B - \\mu_A \\cdot \\mu_B'],
                  ['Complemento (NOT)', '\\bar{A} = 1 - A = \\{0,1\\}', '\\mu_{\\bar{A}}(x) = 1 - \\mu_A(x)', '1 - \\mu_A(x) \\text{ (igual)}'],
                ].map(([op, cl, fmm, fp]) => (
                  <tr key={op}>
                    <td style={{ ...S.td, fontWeight: 600, color: '#4a9eed' }}>{op}</td>
                    <td style={S.td}><InlineMath math={cl} /></td>
                    <td style={S.td}><InlineMath math={fmm} /></td>
                    <td style={{ ...S.td, color: 'var(--text-secondary)', fontSize: '0.83rem' }}><InlineMath math={fp} /></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div style={S.section}>
          ...
          <div style={S.diagram}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem', fontSize: '0.85rem' }}>
              {[
                { rule: 'SE temperatura É Fria ENTÃO ventoinha É Lenta', cond: 'μ_fria > 0', color: '#4a9eed' },
                { rule: 'SE temperatura É Morna ENTÃO ventoinha É Média', cond: 'μ_morna > 0', color: '#4a9eed' },
                { rule: 'SE temperatura É Quente ENTÃO ventoinha É Rápida', cond: 'μ_quente > 0', color: '#4a9eed' },
                { rule: 'SE temperatura É Quente E humidade É Alta ENTÃO ventoinha É Máxima', cond: 'μ_quente>0 AND μ_alta>0', color: '#4a9eed' },
              ].map(({ rule, cond, color }) => (
            ...
            <table style={S.table}>
              ...
              <tbody>
                {[
                  ...
                  <tr key={a}><td style={{ ...S.td, fontWeight: 600, color: 'var(--text-secondary)' }}>{a}</td><td style={{ ...S.td, color: '#4a9eed' }}>{m}</td><td style={S.td}>{t}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
            ...
            <table style={S.table}>
              ...
              <tbody>
                {[
                  ...
                  <tr key={a}><td style={{ ...S.td, fontWeight: 600, color: '#4a9eed' }}>{a}</td><td style={S.td}>{c}</td><td style={{ ...S.td, color: '#4a9eed', fontSize: '0.83rem' }}>{v}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
