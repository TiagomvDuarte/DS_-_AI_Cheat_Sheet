...
const DeceptiveFitnessDiagram = () => (
  <div style={S.diagram}>
    ...
    <svg viewBox="0 0 340 210" style={{ maxWidth: '100%', height: 'auto', display: 'block', marginBottom: '1rem' }}>
      ...
      {/* Outer walls */}
      <rect x="20" y="22" width="295" height="7" fill="rgba(249,115,22,0.45)" rx="2"/>{/* top */}
      <rect x="20" y="22" width="7"   height="168" fill="rgba(249,115,22,0.45)" rx="2"/>{/* left */}
      <rect x="20" y="183" width="295" height="7" fill="rgba(249,115,22,0.45)" rx="2"/>{/* bottom */}
      <rect x="308" y="22" width="7" height="75" fill="rgba(249,115,22,0.45)" rx="2"/>{/* right-top (stops at y=97) */}
      <rect x="308" y="135" width="7" height="55" fill="rgba(249,115,22,0.45)" rx="2"/>{/* right-bottom (starts at y=135) */}
      {/* EXIT gap: y=97 to y=135 on right wall */}

      {/* Interior T-divider */}
      <rect x="85" y="29" width="7" height="76" fill="rgba(249,115,22,0.45)" rx="2"/>{/* vertical: left wall of upper chamber */}
      <rect x="85" y="98" width="196" height="7" fill="rgba(249,115,22,0.45)" rx="2"/>{/* horizontal: floor of upper chamber */}

      {/* TRAP box (dashed) — inside upper chamber */}
      <rect x="160" y="40" width="60" height="38" rx="5" fill="rgba(249,115,22,0.10)" stroke="#f97316" strokeWidth="2" strokeDasharray="5,3"/>
      <text x="190" y="58" textAnchor="middle" fill="#f97316" fontSize="9" fontWeight="700">TRAP</text>
      <text x="190" y="70" textAnchor="middle" fill="#f97316" fontSize="7.5">dead end</text>

      {/* EXIT label at right gap */}
      <rect x="296" y="97" width="36" height="38" rx="4" fill="rgba(249,115,22,0.15)" stroke="#f97316" strokeWidth="2"/>
      <text x="314" y="113" textAnchor="middle" fill="#f97316" fontSize="8" fontWeight="700">SAÍDA</text>
      <text x="314" y="125" textAnchor="middle" fill="#f97316" fontSize="7">real</text>

      {/* Agent A — lower corridor */}
      <circle cx="50" cy="148" r="11" fill="rgba(249,115,22,0.2)" stroke="#f97316" strokeWidth="2"/>
      <text x="50" y="152" textAnchor="middle" fill="#f97316" fontSize="9" fontWeight="700">A</text>

      {/* Dashed path: fitness (wrong) — up left gap into upper chamber → TRAP */}
      <path d="M 50,137 L 50,35 Q 75,32 160,55" fill="none" stroke="#f97316" strokeWidth="1.8" strokeDasharray="5,3" markerEnd="url(#arr-ns)"/>
      <text x="22" y="100" fill="#f97316" fontSize="7.5" fontWeight="700" transform="rotate(-90,30,90)">fitness ↑ (errado)</text>

      {/* Solid path: novelty — right along lower corridor → EXIT */}
      <path d="M 61,148 L 280,148 L 296,116" fill="none" stroke="#f97316" strokeWidth="2" markerEnd="url(#arr-ns2)"/>
      <text x="170" y="170" textAnchor="middle" fill="#f97316" fontSize="7.5" fontWeight="700">novidade → explora → encontra saída real</text>
    </svg>
    ...
    <svg viewBox="0 0 500 175" style={{ maxWidth: '100%', height: 'auto', display: 'block' }}>
      ...
      <text x="205" y="46" textAnchor="middle" fill="#f97316" fontSize="9" fontWeight="700">Fitness</text>
      <text x="395" y="46" textAnchor="middle" fill="#f97316" fontSize="9" fontWeight="700">Novelty Search</text>
      {[
        ...
        <g key={y}>
          ...
          <text x="130" y={y} fill="#f97316" fontSize="9">{f}</text>
          <text x="310" y={y} fill="#f97316" fontSize="9">{n}</text>
        </g>
      ))}
    </svg>
  </div>
    ...
    <svg viewBox="0 0 700 290" style={{ maxWidth: '100%', height: 'auto' }}>
      ...
      {Array.from({ length: 5 }, (_, row) =>
        Array.from({ length: 5 }, (_, col) => {
          ...
          const color = fitness ? `rgba(249,115,22,${0.1 + parseFloat(fitness) * 0.5})` : 'rgba(100,100,100,0.05)';
          const strokeC = fitness ? '#f97316' : 'var(--card-border)';
          return (
            <g key={`${row}-${col}`}>
              <rect x={x} y={y} width={104} height={40} rx="5"
                fill={color} stroke={strokeC} strokeWidth={fitness ? 1.5 : 1}/>
              {fitness
                ? <text x={x + 52} y={y + 25} textAnchor="middle" fill="#f97316" fontSize="10" fontWeight="700">f={fitness}</text>
      ...
      <text x={80 + 3 * 110 + 52} y={28 + 1 * 46 - 5} textAnchor="middle" fill="#f97316" fontSize="8" fontWeight="700">melhor deste nicho</text>
  ...
  return (
    <div style={{ padding: '2rem 1rem' }}>
      <div style={S.page}>
        ...
        <p style={S.lead}>E se o problema for o próprio objectivo? Novelty Search abandona o fitness e maximiza diversidade comportamental. MAP-Elites combina qualidade e diversidade numa grelha de nichos. Juntos formam o paradigma Quality-Diversity — útil sempre que o fitness é deceptivo ou quando se quer iluminar o espaço de soluções possíveis.</p>
        ...
        <div style={S.section}>
          ...
          <p style={S.p}>O Novelty Search (Lehman & Stanley, 2011) propõe uma solução radical: <strong>ignorar completamente o objectivo</strong>. Em vez de maximizar f(x), maximiza-se a <em>novidade</em> do comportamento — quão diferente é este indivíduo de tudo o que já foi visto.</p>

          <h3 style={S.h3}>Definição de Novidade</h3>
          <p style={S.p}>Cada indivíduo é caracterizado pelo seu <strong>vector de comportamento</strong> b(x) — uma descrição compacta do que o indivíduo faz (e.g., posição final no labirinto, sequência de movimentos, forças aplicadas). A novidade de x é a distância média aos k vizinhos mais próximos no arquivo de comportamentos já visitados:</p>
          <div style={S.math}>
          ...
          <p style={S.p}>O arquivo cresce ao longo da evolução: indivíduos suficientemente novos (ρ(x) &gt; ρ_min) são adicionados. No final, o arquivo contém uma cobertura densa do espaço de comportamentos — e o objectivo, se existir solução, tende a ser encontrado "por acidente" durante a exploração sistemática.</p>
              ...
              <tbody>
                {[
                  ...
                  <tr key={a}>
                    ...
                    <td style={{ ...S.td, color: '#f97316' }}>{ea}</td>
                    <td style={{ ...S.td, color: '#f97316' }}>{ns}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div style={S.section}>
          ...
          <div style={{ background: 'var(--bg-secondary)', border: '1px solid var(--card-border)', borderRadius: 8, padding: '1rem', marginBottom: '1rem', fontFamily: 'monospace', fontSize: '0.85rem', lineHeight: 2, color: 'var(--text-primary)' }}>
            <div style={{ color: '#f97316', fontWeight: 700 }}>MAP-Elites</div>
              ...
              <tbody>
                {[
                  ...
                  <tr key={p}>
                    ...
                    <td style={S.td}>{ns}</td>
                    <td style={{ ...S.td, color: '#f97316' }}>{me}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div style={S.section}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1.5rem' }}>
            {[
              { title: 'Quando usar QD', color: '#f97316', items: ['O problema tem múltiplas soluções válidas com comportamentos distintos', 'Robótica adaptativa — necessidade de fallback comportamental', 'Design generativo — explorar o espaço de designs possíveis', 'O fitness é deceptivo ou tem muitos óptimos locais equivalentes', 'Queremos compreender o espaço de soluções, não apenas a melhor'] },
              { title: 'Quando NÃO usar QD', color: '#f97316', items: ['Existe apenas uma solução válida e o fitness é claro', 'O espaço de comportamentos não é definível ou é trivial', 'Recursos computacionais muito limitados (QD requer muitas avaliações)', 'O problema tem estrutura exploitável por gradiente (diferenciável)'] },
          ...
          <table style={S.table}>
            ...
            <tbody>
              {[
                ...
                <tr key={alg}>
                  <td style={{ ...S.td, fontWeight: 700, color: '#f97316' }}>{alg}</td>
        ...
        <div style={S.highlight}>
          <strong>Síntese — Quality-Diversity:</strong>
          <ul style={{ margin: '0.5rem 0 0', paddingLeft: '1.2rem', fontSize: '0.9rem', color: 'var(--text-primary)', lineHeight: 1.9 }}>
            <li><strong>Fitness deceptivo</strong>: o gradiente de fitness aponta na direcção errada — qualquer EA objectivo fica preso.</li>
            <li><strong>Novelty Search</strong>: substituir fitness por novidade ρ(x) = média das distâncias aos k vizinhos no arquivo comportamental. Explora sem objectivo.</li>
            <li><strong>MAP-Elites</strong>: grelha N-dimensional de nichos comportamentais. Cada célula guarda o melhor indivíduo para aquele comportamento. Um indivíduo só substitui o ocupante se tiver fitness superior.</li>
            <li><strong>Quality-Diversity</strong>: paradigma geral — diversidade comportamental + qualidade por nicho. Output é um repertório, não uma solução única.</li>
            <li>Aplicação chave: robótica adaptativa onde o robô selecciona em tempo real o comportamento certo para o contexto (e.g., perna avariada).</li>
          </ul>
        </div>
      </div>
        <hr style={S.divider} />
        <div style={S.section}>
          ...
          <div style={S.highlight}>
            <ul style={{paddingLeft:'1.2rem', margin:0}}>
                            <li style={{marginBottom:"0.4rem"}}><strong>O Problema do Fitness Deceptivo</strong> — conceito central desta lecture.</li>
              <li style={{marginBottom:"0.4rem"}}><strong>Novelty Search — Explorar em vez de Optimizar</strong> — conceito central desta lecture.</li>
              <li style={{marginBottom:"0.4rem"}}><strong>MAP-Elites — Iluminar o Espaço de Soluções</strong> — conceito central desta lecture.</li>
              <li style={{marginBottom:"0.4rem"}}><strong>Quality-Diversity como Paradigma</strong> — conceito central desta lecture.</li>
            </ul>
          </div>
        </div>
    </div>
