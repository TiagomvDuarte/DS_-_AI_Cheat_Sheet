...
const TimelineDiagram = () => (
  <div style={S.diagram}>
    ...
    <div style={{ position: 'relative', padding: '0 1rem' }}>
      ...
      {[
        { year: '2002', name: 'NEAT', color: '#f97316', desc: 'Innovation numbers + especiação + topologia mínima. Base de toda a família. Resolve competing conventions, protege inovações e começa simples. Stanley & Miikkulainen, U-Texas.' },
        { year: '2007', name: 'HyperNEAT', color: '#f97316', desc: 'Codificação indirecta via CPPNs. Os pesos são gerados por um padrão geométrico — escala para redes com milhões de conexões. Explora regularidade geométrica do problema.' },
        { year: '2012', name: 'ES-HyperNEAT', color: '#f97316', desc: 'Evolvable Substrate. O substrato (posição dos neurónios) também evolui com base na variância do CPPN — não é mais definido pelo utilizador. Estrutura neuronal emerge do processo evolutivo.' },
        { year: '2017', name: 'CoDeepNEAT', color: '#f97316', desc: 'Co-evolução de módulos e blueprints para arquitecturas de deep learning. Permite evoluir automaticamente arquitecturas CNN + LSTM + FC. Mirhoseini et al., Google Brain.' },
        { year: '2017', name: 'OpenAI ES', color: '#f97316', desc: 'Evolution Strategies escalável sem topologia. Perturbações gaussianas + estimativa de gradiente distribuída. Compete com PPO em MuJoCo com 1000 CPUs em paralelo.' },
        { year: '2018+', name: 'MAP-Elites / QD', color: '#f97316', desc: 'Quality-Diversity: explorar o espaço de comportamentos e manter o melhor indivíduo de cada "nicho". Ilhas de diversidade ao invés de convergência. Illumination algorithm.' },
...
const HyperNEATDetail = () => (
  <div style={{ ...S.diagram, textAlign: 'center' }}>
    ...
    <svg viewBox="0 0 600 160" style={{ maxWidth: '100%', height: 'auto' }}>
      <defs>
        <marker id="arr7" markerWidth="6" markerHeight="6" refX="5" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill="#f97316"/></marker>
        <marker id="arr7b" markerWidth="6" markerHeight="6" refX="5" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill="#f97316"/></marker>
      </defs>

      <rect x={10} y={40} width={145} height={80} rx="8" fill="rgba(249,115,22,0.10)" stroke="#f97316" strokeWidth="1.5"/>
      <text x={82} y={60} textAnchor="middle" fill="#f97316" fontSize="9" fontWeight="700">CPPN (evoluído pelo NEAT)</text>
      ...
      <text x={82} y={97} textAnchor="middle" fill="#f97316" fontSize="8" fontWeight="700">output: w = peso da conexão</text>
      <text x={82} y={111} textAnchor="middle" fill="var(--text-secondary)" fontSize="7">1 CPPN codifica toda a rede alvo</text>

      <line x1={155} y1={80} x2={195} y2={80} stroke="#f97316" strokeWidth="1.5" markerEnd="url(#arr7)"/>
      ...
      <rect x={200} y={20} width={175} height={120} rx="8" fill="rgba(249,115,22,0.10)" stroke="#f97316" strokeWidth="1.5"/>
      <text x={287} y={38} textAnchor="middle" fill="#f97316" fontSize="9" fontWeight="700">Substrate (ANN alvo)</text>
      ...
      {[
        ...
        <circle key={i} cx={x} cy={y} r={8} fill="rgba(249,115,22,0.12)" stroke="#f97316" strokeWidth="1.2"/>
      ))}
      {[[225,75,285,70],[225,100,285,100],[225,125,285,130],[285,70,345,85],[285,100,345,110]].map(([x1,y1,x2,y2], i) => (
        <line key={i} x1={x1+8} y1={y1} x2={x2-8} y2={y2} stroke="#f97316" strokeWidth="0.8" opacity="0.6"/>
      ))}

      <line x1={375} y1={80} x2={415} y2={80} stroke="#f97316" strokeWidth="1.5" markerEnd="url(#arr7b)"/>
      <text x={395} y={72} textAnchor="middle" fill="var(--text-secondary)" fontSize="7">w gerado</text>

      <rect x={420} y={25} width={165} height={110} rx="8" fill="rgba(249,115,22,0.10)" stroke="#f97316" strokeWidth="1.5"/>
      <text x={502} y={43} textAnchor="middle" fill="#f97316" fontSize="9" fontWeight="700">Insight geométrico</text>
      ...
      <text x={432} y={124} fill="#f97316" fontSize="7.5" fontWeight="700">automaticamente em todas as conexões</text>
    </svg>
  </div>
);
  ...
  <div style={{ ...S.diagram, textAlign: 'center' }}>
    ...
    <svg viewBox="0 0 740 320" style={{ maxWidth: '100%', height: 'auto' }}>
      ...
      <rect x="10" y="10" width="195" height="295" rx="10"
        fill="rgba(249,115,22,0.10)" stroke="#f97316" strokeWidth="1.5" strokeDasharray="6,3"/>
      <text x="107" y="30" textAnchor="middle" fill="#f97316" fontSize="10" fontWeight="700">POPULAÇÃO DE MÓDULOS</text>
      <text x="107" y="44" textAnchor="middle" fill="var(--text-secondary)" fontSize="8">evoluídos pelo NEAT</text>

      {/* Módulo A — Conv */}
      <rect x="24" y="54" width="167" height="52" rx="7" fill="rgba(249,115,22,0.10)" stroke="#f97316" strokeWidth="1.5"/>
      <text x="107" y="72" textAnchor="middle" fill="#f97316" fontSize="9" fontWeight="700">MÓDULO A</text>
      ...
      {/* Módulo B — LSTM */}
      <rect x="24" y="118" width="167" height="52" rx="7" fill="rgba(249,115,22,0.10)" stroke="#f97316" strokeWidth="1.5"/>
      <text x="107" y="136" textAnchor="middle" fill="#f97316" fontSize="9" fontWeight="700">MÓDULO B</text>
      ...
      {/* Módulo C — FC */}
      <rect x="24" y="182" width="167" height="52" rx="7" fill="rgba(249,115,22,0.10)" stroke="#f97316" strokeWidth="1.5"/>
      <text x="107" y="200" textAnchor="middle" fill="#f97316" fontSize="9" fontWeight="700">MÓDULO C</text>
      ...
      {/* Módulo D — Attention */}
      <rect x="24" y="246" width="167" height="30" rx="7" fill="rgba(249,115,22,0.10)" stroke="#f9731630" strokeWidth="1" strokeDasharray="4,2"/>
      <text x="107" y="265" textAnchor="middle" fill="#f97316" fontSize="8">MÓDULO D  (novo, a avaliar…)</text>

      {/* ── POPULAÇÃO DE BLUEPRINTS (centro) ── */}
      <rect x="270" y="10" width="195" height="295" rx="10"
        fill="rgba(249,115,22,0.10)" stroke="#f97316" strokeWidth="1.5" strokeDasharray="6,3"/>
      <text x="367" y="30" textAnchor="middle" fill="#f97316" fontSize="10" fontWeight="700">POPULAÇÃO DE BLUEPRINTS</text>
      <text x="367" y="44" textAnchor="middle" fill="var(--text-secondary)" fontSize="8">grafos de montagem</text>

      {/* Blueprint 1 */}
      <rect x="284" y="54" width="167" height="100" rx="7" fill="rgba(249,115,22,0.10)" stroke="#f97316" strokeWidth="1.5"/>
      <text x="367" y="70" textAnchor="middle" fill="#f97316" fontSize="9" fontWeight="700">Blueprint 1</text>
      {/* mini graph: A → B → C */}
      {[{lbl:'A',cx:320,cy:100},{lbl:'B',cx:367,cy:100},{lbl:'C',cx:414,cy:100}].map(({lbl,cx,cy})=>(
        <g key={'bp1'+lbl}>
          <circle cx={cx} cy={cy} r={13} fill="rgba(249,115,22,0.10)" stroke="#f97316" strokeWidth="1.5"/>
          <text x={cx} y={cy+4} textAnchor="middle" fill="#f97316" fontSize="9" fontWeight="700">{lbl}</text>
        </g>
      ))}
      <line x1="333" y1="100" x2="354" y2="100" stroke="#f97316" strokeWidth="1.2" markerEnd="url(#arr-cd)"/>
      <line x1="380" y1="100" x2="401" y2="100" stroke="#f97316" strokeWidth="1.2" markerEnd="url(#arr-cd)"/>
      ...
      {/* Blueprint 2 */}
      <rect x="284" y="168" width="167" height="105" rx="7" fill="rgba(249,115,22,0.10)" stroke="#f97316" strokeWidth="1.5"/>
      <text x="367" y="184" textAnchor="middle" fill="#f97316" fontSize="9" fontWeight="700">Blueprint 2</text>
      ...
      {[{lbl:'A',cx:315,cy:218},{lbl:'B',cx:367,cy:205},{lbl:'C',cx:367,cy:232},{lbl:'out',cx:415,cy:218}].map(({lbl,cx,cy})=>(
        <g key={'bp2'+lbl}>
          <circle cx={cx} cy={cy} r={lbl==='out'?11:13} fill="rgba(249,115,22,0.10)" stroke="#f97316" strokeWidth="1.5"/>
          <text x={cx} y={cy+4} textAnchor="middle" fill="#f97316" fontSize={lbl==='out'?7:9} fontWeight="700">{lbl}</text>
        </g>
      ))}
      <line x1="328" y1="212" x2="354" y2="207" stroke="#f97316" strokeWidth="1.2" markerEnd="url(#arr-cd)"/>
      <line x1="328" y1="224" x2="354" y2="230" stroke="#f97316" strokeWidth="1.2" markerEnd="url(#arr-cd)"/>
      <line x1="380" y1="207" x2="402" y2="215" stroke="#f97316" strokeWidth="1.2" markerEnd="url(#arr-cd)"/>
      <line x1="380" y1="230" x2="402" y2="221" stroke="#f97316" strokeWidth="1.2" markerEnd="url(#arr-cd)"/>
      ...
      <rect x="530" y="10" width="200" height="295" rx="10"
        fill="rgba(249,115,22,0.10)" stroke="#f97316" strokeWidth="1.5" strokeDasharray="6,3"/>
      <text x="630" y="30" textAnchor="middle" fill="#f97316" fontSize="10" fontWeight="700">REDE FINAL MONTADA</text>
      <text x="630" y="44" textAnchor="middle" fill="var(--text-secondary)" fontSize="8">Blueprint 2 + módulos reais</text>

      {[
        {lbl:'Módulo A', sub:'Conv 3×3→ReLU→Pool', cy:75,  color:'#f97316'},
        {lbl:'Módulo B', sub:'LSTM 128→Drop 0.3',  cy:135, color:'#f97316'},
        {lbl:'Módulo C', sub:'FC 256→BN→ReLU',     cy:195, color:'#f97316'},
        {lbl:'Output',   sub:'Softmax',             cy:255, color:'#f97316'},
      ...
      <line x1="630" y1="93"  x2="630" y2="113" stroke="#f97316" strokeWidth="1.5" markerEnd="url(#arr-cd2)"/>
      <line x1="630" y1="153" x2="630" y2="173" stroke="#f97316" strokeWidth="1.5" markerEnd="url(#arr-cd2)"/>
      <line x1="630" y1="213" x2="630" y2="233" stroke="#f97316" strokeWidth="1.5" markerEnd="url(#arr-cd2)"/>

      {/* ── Setas de co-evolução ── */}
      {/* módulos → blueprints */}
      <path d="M 205,80 Q 237,80 270,80" fill="none" stroke="#f97316" strokeWidth="1.5" markerEnd="url(#arr-cd)" strokeDasharray="5,3"/>
      <path d="M 205,144 Q 237,144 270,155" fill="none" stroke="#f97316" strokeWidth="1.5" markerEnd="url(#arr-cd)" strokeDasharray="5,3"/>
      <path d="M 205,208 Q 237,208 270,220" fill="none" stroke="#f97316" strokeWidth="1.5" markerEnd="url(#arr-cd)" strokeDasharray="5,3"/>

      {/* blueprint vencedor → rede final */}
      <path d="M 465,218 Q 497,218 530,140" fill="none" stroke="#f97316" strokeWidth="2" markerEnd="url(#arr-cd2)"/>
      <text x="497" y="195" fill="#f97316" fontSize="8" fontWeight="700" textAnchor="middle">monta</text>
...
const CPPNActivations = () => (
  <div style={S.diagram}>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '0.75rem' }}>
      {[
        { name: 'sin(x)', color: '#f97316', pattern: 'Padrão ondulado periódico — simetrias bilaterais, estruturas repetidas. Ideal para redes com topologia segmentada (pernas de robô).' },
        { name: 'gauss(x)', color: '#f97316', pattern: 'Concentração radial — conexões fortes perto do centro, fracas na periferia. Útil para campos receptivos locais.' },
        { name: 'linear(x)', color: '#f97316', pattern: 'Gradiente monotónico — pesos crescem/decrescem com a distância. Simula topografias neuronais simples.' },
        { name: 'sigmoid(x)', color: '#f97316', pattern: 'Transição suave — regiões de pesos altos e baixos separadas por fronteira sigmoidal. Boa para separação de camadas.' },
  ...
  return (
    <div style={{ padding: '2rem 1rem' }}>
      <div style={S.page}>
        ...
        <p style={S.lead}>O NEAT de 2002 estabeleceu os princípios fundamentais de neuroevolução com topologia variável. Nas duas décadas seguintes, uma família de algoritmos resolveu os seus limites: HyperNEAT, ES-HyperNEAT, CoDeepNEAT e OpenAI ES.</p>
            ...
            <table style={S.table}>
              ...
              <tbody>
                {[
                  ...
                  <tr key={a}><td style={{ ...S.td, fontWeight: 600, color: 'var(--text-secondary)' }}>{a}</td><td style={S.td}>{ppo}</td><td style={{ ...S.td, color: '#f97316' }}>{es}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
            ...
            <table style={S.table}>
              ...
              <tbody>
                {[
                  ...
                  <tr key={alg}><td style={{ ...S.td, fontWeight: 700, color: '#f97316' }}>{alg}</td><td style={S.td}>{top}</td><td style={S.td}>{cod}</td><td style={{ ...S.td, color: 'var(--text-secondary)', fontSize: '0.83rem' }}>{esc}</td><td style={{ ...S.td, color: 'var(--text-secondary)', fontSize: '0.83rem' }}>{use}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
        ...
        <div style={S.section}>
          ...
          <div style={S.highlight}>
            <ul style={S.ul}>
            <li><strong>NEAT (2002)</strong>: base — innovation numbers + especiação + topologia mínima. Resolve os 3 problemas fundamentais da neuroevolução.</li>
            <li><strong>HyperNEAT (2007)</strong>: codificação indirecta via CPPNs. 100 pesos codificam 100.000 conexões se o problema tem regularidade geométrica.</li>
            <li><strong>ES-HyperNEAT (2012)</strong>: substrato evoluível — onde colocar os neurónios emerge da variância do CPPN. Neurogenese computacional.</li>
            <li><strong>CoDeepNEAT (2017)</strong>: co-evolução de módulos e blueprints para arquitecturas deep learning modulares.</li>
            <li><strong>OpenAI ES (2017)</strong>: estimativa de gradiente sem backprop, trivialmente paralela. Simples, escalável, sem topologia.</li>
            <li><strong>Novelty Search e MAP-Elites</strong>: ver módulo dedicado de Quality-Diversity.</li>
          </ul>
          </div>
        </div>
      </div>
