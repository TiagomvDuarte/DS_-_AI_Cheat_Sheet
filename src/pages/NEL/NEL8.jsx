  ...
  <div style={{ ...S.diagram, textAlign: 'center' }}>
    ...
    <svg viewBox="0 0 600 180" style={{ maxWidth: '100%', height: 'auto' }}>
      <defs>
        ...
        <marker id="arr8b" markerWidth="6" markerHeight="6" refX="5" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill="#f97316"/></marker>
      </defs>

      {/* RL clássico */}
      <text x="140" y="18" textAnchor="middle" fill="var(--text-secondary)" fontSize="9" fontWeight="700">RL CLÁSSICO (Gradient-based)</text>
      <rect x="10" y="25" width="260" height="130" rx="8" fill="rgba(249,115,22,0.10)" stroke="var(--text-secondary)" strokeWidth="1"/>
      {[{x:60,y:60,l:'Agente\n(policy π)'},{x:175,y:60,l:'Ambiente'},{x:175,y:125,l:'Recompensa\nr_t'},{x:60,y:125,l:'Actualização\n∇θJ(π)'}].map(({x,y,l})=>(
        <g key={l}>
          <rect x={x-35} y={y-15} width={70} height={30} rx="5" fill="rgba(249,115,22,0.10)" stroke="#f97316" strokeWidth="1.2"/>
          {l.split('\n').map((line,i)=>(
            <text key={i} x={x} y={y+i*11-3} textAnchor="middle" fill="#f97316" fontSize="7.5" fontWeight="600">{line}</text>
          ))}
        </g>
      ))}
      <line x1={95} y1={60} x2={138} y2={60} stroke="#f97316" strokeWidth="1.2" markerEnd="url(#arr8)"/>
      <text x="118" y="54" textAnchor="middle" fill="var(--text-secondary)" fontSize="7">acção a_t</text>
      <line x1={175} y1={75} x2={175} y2={110} stroke="#f97316" strokeWidth="1.2" markerEnd="url(#arr8)"/>
      <line x1={140} y1={125} x2={97} y2={125} stroke="#f97316" strokeWidth="1.2" markerEnd="url(#arr8)"/>
      <line x1={60} y1={110} x2={60} y2={76} stroke="#f97316" strokeWidth="1.2" markerEnd="url(#arr8)"/>
      <text x="140" y="162" textAnchor="middle" fill="#f97316" fontSize="8">Actualiza 1 política por gradiente. Precisa de gradiente diferenciável.</text>

      {/* EvoRL */}
      <text x="450" y="18" textAnchor="middle" fill="var(--text-secondary)" fontSize="9" fontWeight="700">EvoRL (Population-based)</text>
      <rect x="320" y="25" width="260" height="130" rx="8" fill="rgba(249,115,22,0.10)" stroke="var(--text-secondary)" strokeWidth="1"/>
      {[{x:355,y:55,l:'π₁'},{x:395,y:55,l:'π₂'},{x:435,y:55,l:'π₃'},{x:475,y:55,l:'π₄'},{x:515,y:55,l:'π₅'}].map(({x,y,l})=>(
        <g key={l}>
          <circle cx={x} cy={y} r={14} fill="rgba(249,115,22,0.15)" stroke="#f97316" strokeWidth="1.5"/>
          <text x={x} y={y+4} textAnchor="middle" fill="#f97316" fontSize="8" fontWeight="700">{l}</text>
        </g>
      ))}
      {[{x:355,y:100,l:'r₁=12'},{x:395,y:100,l:'r₂=45'},{x:435,y:100,l:'r₃=8'},{x:475,y:100,l:'r₄=67'},{x:515,y:100,l:'r₅=23'}].map(({x,y,l})=>(
        <g key={l}>
          <text x={x} y={y} textAnchor="middle" fill={parseInt(l.split('=')[1]) > 40 ? '#f97316' : 'var(--text-secondary)'} fontSize="7.5">{l}</text>
        </g>
      ))}
      <text x="450" y="130" textAnchor="middle" fill="#f97316" fontSize="8" fontWeight="700">Selecção + Crossover + Mutação → nova geração</text>
      <text x="450" y="162" textAnchor="middle" fill="#f97316" fontSize="8">Avalia N políticas em paralelo. Fitness = recompensa total do episódio.</text>
    </svg>
  </div>
);
...
const ComparisonExplorer = () => {
  ...
  const items = [
    {
      name: 'EvoRL vs PPO', color: '#f97316',
    ...
    {
      name: 'Neuroevolução (NEAT/ES)', color: '#f97316',
    ...
    {
      name: 'Evolutionary RL Híbrido', color: '#f97316',
    ...
    <div style={S.diagram}>
      ...
      <div style={{ background: 'var(--bg-primary)', borderRadius: 10, padding: '1.25rem', border: `1.5px solid ${it.color}30` }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem', fontSize: '0.85rem', marginBottom: '0.75rem' }}>
          ...
          <div style={{ background: 'var(--bg-secondary)', borderRadius: 8, padding: '0.75rem' }}>
            <div style={{ fontWeight: 700, color: '#f97316', fontSize: '0.78rem', textTransform: 'uppercase', marginBottom: '0.35rem' }}>Quando RL Clássico é Melhor</div>
  ...
  return (
    <div style={{ padding: '2rem 1rem' }}>
      <div style={S.page}>
        ...
        <p style={S.lead}>A Aprendizagem por Reforço (RL) clássica usa gradientes para optimizar uma política. A Aprendizagem por Reforço Evolutiva (EvoRL) aplica algoritmos evolutivos — populações, selecção, crossover, mutação — directamente sobre políticas. A abordagem evolutiva não precisa de gradiente, é trivialmente paralelizável, e é robusta a recompensas esparsas. Em 2017, a OpenAI demonstrou que Evolution Strategies competia com os melhores métodos de RL em MuJoCo, com muito mais simplicidade de implementação.</p>

        <div style={S.section}>
          <h2 style={S.h2}>1. RL Clássico vs Aprendizagem Evolutiva</h2>
          <p style={S.p}>No RL clássico (PPO, A3C, SAC, TD3), o agente aprende uma política π_θ actualizando os parâmetros θ na direcção do gradiente de retorno esperado. O gradiente é estimado a partir das trajectórias geradas pela própria política. O processo é iterativo: amostragem → estimativa de gradiente → actualização de θ.</p>
          <p style={S.p}>Na EvoRL, mantém-se uma população de políticas &#123;π₁, π₂, ..., πₙ&#125;. Cada política é avaliada num ambiente por rollout completo. O fitness é a recompensa total acumulada no episódio. As políticas de maior fitness são seleccionadas, cruzadas e mutadas para produzir a geração seguinte. Não há gradiente, não há backpropagation.</p>
          <EvoRLDiagram />
        </div>
              ...
              <tbody>
                {[
                  ...
                  <tr key={alg}>
                    ...
                    <td style={{ ...S.td, color: '#f97316', fontSize: '0.83rem' }}>{pts}</td>
            ...
            <table style={S.table}>
              ...
              <tbody>
                {[
                  ...
                  <tr key={d}><td style={{ ...S.td, fontWeight: 600, color: '#f97316' }}>{d}</td><td style={S.td}>{p}</td><td style={{ ...S.td, color: '#f97316', fontSize: '0.83rem' }}>{r}</td><td style={{ ...S.td, fontSize: '0.82rem', color: 'var(--text-secondary)' }}>{res}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
        ...
        <div style={S.section}>
          ...
          <div style={S.highlight}>
            <ul style={S.ul}>
            <li>EvoRL: população de políticas evoluídas por selecção + crossover + mutação. Fitness = recompensa total do episódio. Sem gradiente.</li>
            <li>Vantagem chave: não precisa de gradiente → funciona com qualquer política (não-diferenciável). Paralelismo trivial.</li>
            <li>OpenAI ES: perturbações gaussianas + estimativa de gradiente distribuída. Comunicação O(1) por worker — escala perfeitamente.</li>
            <li>ERL combina o melhor dos dois mundos: exploração evolutiva para recompensa esparsa + eficiência de gradiente para refinamento.</li>
            <li>NEAT para RL: descobre automaticamente a arquitectura e os pesos. Ideal quando a estrutura da política é desconhecida.</li>
            <li>EvoRL supera RL clássico em: recompensas esparsas, políticas não-diferenciáveis, landscape deceptivo, e quando o paralelismo é abundante.</li>
          </ul>
          </div>
        </div>
      </div>
