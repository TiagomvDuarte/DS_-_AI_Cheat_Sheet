...
function ComparisonSVG() {
  return (
    <svg viewBox="0 0 700 260" style={{ width: '100%', height: 'auto' }}>
      {/* Frequentist box */}
      <rect x="10" y="10" width="320" height="240" rx="10" fill="rgba(249,115,22,0.07)" stroke="#f59e0b" strokeWidth="1.5" />
      <text x="170" y="38" textAnchor="middle" fontSize="13" fontWeight="700" fill="#f59e0b">FREQUENTISTA</text>
      {/* bullet rows */}
      {[
        ...
        <g key={k} transform={`translate(20, ${55 + i * 30})`}>
          <text fontSize="11" fontWeight="600" fill="#f59e0b">{k}:</text>
...
function BeliefUpdateSVG() {
  return (
    <svg viewBox="0 0 680 110" style={{ width: '100%', height: 'auto' }}>
      {[
        { x: 20, label: 'Prior P(θ)', color: '#f59e0b', fill: 'rgba(245,158,11,0.12)', desc: 'Antes dos dados' },
        { x: 240, label: 'Dados observados', color: '#fb923c', fill: 'rgba(251,146,60,0.12)', desc: 'Nova evidência' },
        { x: 460, label: 'Posterior P(θ|data)', color: '#f97316', fill: 'rgba(249,115,22,0.12)', desc: 'Crença atualizada' },
      ].map((item, i) => (
        <g key={item.label}>
          ...
          {i < 2 && (
            <path d={`M ${item.x + 182} 52 L ${item.x + 228} 52`} stroke="var(--text-secondary)" strokeWidth="1.5" fill="none" />
          )}
          {i < 2 && (
            <polygon points={`${item.x + 228},48 ${item.x + 238},52 ${item.x + 228},56`} fill="var(--text-secondary)" />
          )}
        </g>
      ))}
...
function BetaShapesSVG() {
  ...
  const betaCurves = [
    ...
    { a: 2, b: 5, stroke: '#f59e0b', dash: '', label: 'Beta(2,5) — Assimétrica esq.' },
    { a: 5, b: 2, stroke: '#fb923c', dash: '', label: 'Beta(5,2) — Assimétrica dir.' },
    { a: 5, b: 5, stroke: '#f97316', dash: '', label: 'Beta(5,5) — Simétrica' },
    { a: 0.5, b: 0.5, stroke: '#fbbf24', dash: '6,3', label: 'Beta(0.5,0.5) — Jeffreys' },
  ];
...
function SequentialUpdateSVG() {
  ...
  const colors = ['var(--text-secondary)', '#f97316', color];
...
function NormalNormalSVG() {
  ...
  return (
    <svg viewBox="0 0 620 230" style={{ width: '100%', height: 'auto' }}>
      ...
      <path d={getPath(0, 2.5, '#f59e0b', xMin, xMax, W, H, ox, oy)} fill="none" stroke="#f59e0b" strokeWidth="2" strokeDasharray="6,3" />
      <path d={getPath(5, 1.5, '#fb923c', xMin, xMax, W, H, ox, oy)} fill="none" stroke="#fb923c" strokeWidth="2" strokeDasharray="4,3" />
      <path d={getPath(3.5, 1.0, '#f97316', xMin, xMax, W, H, ox, oy)} fill="none" stroke="#f97316" strokeWidth="2.5" />
      {/* x-axis labels */}
      {[-4, -2, 0, 2, 4, 6, 8].map(v => {
      ...
      {/* Legend */}
      {[
        { col: '#f59e0b', dash: '6,3', label: 'Prior N(0, 2.5²) — crença inicial vaga' },
        { col: '#fb923c', dash: '4,3', label: 'Likelihood N(5, 1.5²) — dados observados' },
        { col: '#f97316', dash: '', label: 'Posterior N(3.5, 1.0²) — média ponderada pela precisão' },
      ].map((item, i) => (
...
function TracePlotSVG() {
  ...
  return (
    <svg viewBox="0 0 660 215" style={{ width: '100%', height: 'auto', display: 'block', margin: '0 auto' }}>
      ...
      {/* burn-in region */}
      <rect x={ox} y={oy} width={burnX - ox} height={H} fill="rgba(249,115,22,0.10)" />
      <line x1={burnX} y1={oy} x2={burnX} y2={oy + H} stroke="#f97316" strokeWidth="1.5" strokeDasharray="4,3" />
      <text x={(ox + burnX) / 2} y={oy + H + 14} textAnchor="middle" fontSize="9" fill="#f97316">Burn-in (descartar)</text>
      {/* post burn-in region */}
      <rect x={burnX} y={oy} width={W - (burnX - ox)} height={H} fill="rgba(249,115,22,0.10)" />
...
function HierarchicalSVG() {
  ...
  return (
    <svg viewBox="0 0 620 220" style={{ width: '100%', height: 'auto' }}>
      ...
      {/* Hyperprior */}
      <rect x="230" y="30" width="160" height="40" rx="8" fill="rgba(249,115,22,0.10)" stroke="#f97316" strokeWidth="1.5" />
      <text x="310" y="48" textAnchor="middle" fontSize="11" fontWeight="700" fill="#f97316">Hyperprior</text>
      ...
      {groups.map((g, i) => {
        ...
        return (
          <g key={g}>
            <line x1="310" y1="70" x2={gx + 65} y2="110" stroke="#f97316" strokeWidth="1" strokeDasharray="4,3" />
            <rect x={gx} y="110" width="130" height="40" rx="6" fill="rgba(249,115,22,0.10)" stroke={color} strokeWidth="1.5" />
            ...
            {[0, 1, 2].map(j => {
              ...
              return (
                <g key={j}>
                  ...
                  <circle cx={ox2 + 10} cy="185" r="10" fill="rgba(249,115,22,0.10)" stroke={color} strokeWidth="1" />
...
function PoolingSVG() {
  ...
  return (
    <svg viewBox="0 0 620 220" style={{ width: '100%', height: 'auto' }}>
      ...
      {/* No pooling points */}
      {noPool.map((v, i) => (
        <circle key={i} cx={toX(i)} cy={toY(v)} r="5" fill="#f59e0b" fillOpacity="0.85" />
      ))}
      {/* Partial pooling points */}
      {partial.map((v, i) => (
        <circle key={i} cx={toX(i)} cy={toY(v)} r="5" fill="#f97316" fillOpacity="0.95" />
      ))}
      {/* Shrinkage arrows */}
      {schools.map((s, i) => (
      ...
      <circle cx="220" cy="178" r="5" fill="#f59e0b" fillOpacity="0.85" />
      <text x="228" y="182" fontSize="10" fill="var(--text-primary)">No pooling</text>
      <circle cx="330" cy="178" r="5" fill="#f97316" fillOpacity="0.95" />
...
export default function ST17() {
  return (
    <div style={S.page}>
      <Link to="/statistics" style={S.back}><ArrowLeft size={16} /> Voltar a Statistics</Link>
      <span style={S.tag}>MÓDULO 17</span>
      <h1 style={S.h1}>Estatística Bayesiana</h1>
      <p style={S.lead}>
        A inferência bayesiana oferece uma alternativa poderosa à abordagem frequentista clássica:
        em vez de p-values e intervalos de confiança, obtemos distribuições completas de probabilidade
        sobre os parâmetros, incorporando conhecimento prévio de forma transparente e atualizando
        crenças à medida que novos dados chegam.
      </p>
      ...
      {/* ── SECTION 7 ── */}
      <section style={S.section}>
        <h2 style={S.h2}>7. PyMC — Inferência Bayesiana com Python</h2>
        <p style={S.p}>
          PyMC é a biblioteca Python dominante para inferência bayesiana. Usa o NUTS por padrão,
          integra com ArviZ para diagnósticos e visualizações, e tem API declarativa que mapeia
          naturalmente para a notação matemática.
        </p>
        <h3 style={S.h3}>Posterior Predictive Checks (PPC)</h3>
        <p style={S.p}>
          O PPC é a ferramenta principal de diagnóstico de ajuste em inferência bayesiana. A ideia:
          amostrar dados simulados a partir do posterior — se o modelo for adequado, os dados
          simulados devem parecer com os dados reais. Detecta misspecification do modelo.
        </p>
        <div style={S.highlight}>
          <p style={S.p}><strong>Bayesiano vs OLS na prática:</strong></p>
          <p style={S.p}>
            Com n=100 e priors vagos, as estimativas pontuais são praticamente idênticas aos OLS.
            A diferença está na incerteza: OLS dá um erro padrão assimptótico; Bayesiano dá uma
            distribuição completa sobre cada parâmetro, permitindo perguntas como
            "P(slope {'>'} 2 | data) = ?".
          </p>
        </div>
      </section>
      ...
      {/* ── SECTION 8 ── */}
      <section style={S.section}>
        <h2 style={S.h2}>8. Modelos Hierárquicos (Multilevel)</h2>
        <p style={S.p}>
      ...
      {/* ── SECTION 9 ── */}
      <section style={S.section}>
        <h2 style={S.h2}>9. Contraste: Bayesiano vs Frequentista na Prática</h2>
        <p style={S.p}>
      ...
      <section style={S.section}>
        ...
        <p style={S.p}>
          A inferência bayesiana não é um algoritmo único — é um workflow iterativo de
          construção, validação e refinamento de modelos. O ciclo completo envolve:
          definir o modelo, verificar o prior, ajustar, verificar o posterior, comparar modelos.
        </p>
        <h3 style={S.h3}>Workflow Bayesiano Completo</h3>
        <table style={S.table}>
          <thead>
            <tr>
              <th style={S.th}>Etapa</th>
              <th style={S.th}>O que fazer</th>
              <th style={S.th}>Ferramentas</th>
            </tr>
          </thead>
          <tbody>
            {[
              ['1. Definir modelo', 'Especificar prior + likelihood. Pensar nos dados generativos.', 'PyMC, Stan, Pyro'],
              ['2. Prior predictive check', 'Simular dados do prior. São plausíveis?', 'pm.sample_prior_predictive()'],
              ['3. Ajustar modelo', 'Correr MCMC ou variational inference', 'NUTS, ADVI'],
              ['4. Diagnósticos MCMC', 'R-hat, ESS, trace plots, divergências', 'ArviZ: az.summary(), az.plot_trace()'],
              ['5. Posterior predictive check', 'Simular dados do posterior. Ajuste adequado?', 'pm.sample_posterior_predictive()'],
              ['6. Comparar modelos', 'LOO-CV, WAIC, Bayes Factor', 'az.compare(), az.loo()'],
              ['7. Interpretar', 'Credible intervals, probabilidades diretas, decisões', 'az.hdi(), az.plot_posterior()'],
            ].map(([a, b, c]) => (
              <tr key={a}>
                <td style={S.td}><strong>{a}</strong></td>
                <td style={S.td}>{b}</td>
                <td style={{ ...S.td, fontFamily: 'monospace', fontSize: '0.8rem' }}>{c}</td>
              </tr>
            ))}
          </tbody>
        </table>
        <h3 style={S.h3}>Cheat-sheet: Pares Conjugados</h3>
        <table style={S.table}>
          <thead>
            <tr>
              <th style={S.th}>Prior</th>
          ...
          <tbody>
            {[
              ['Beta(α, β)', 'Binomial(n, θ)', 'Beta(α+k, β+n-k)', 'α+=k, β+=n-k'],
              ['Gamma(α, β)', 'Poisson(λ)', 'Gamma(α+Σx, β+n)', 'α+=Σx, β+=n'],
              ['Normal(μ₀, τ²)', 'Normal(θ, σ²)', 'Normal(μₙ, τₙ²)', 'Média prec. ponderada'],
              ['Dirichlet(α)', 'Multinomial(n, p)', 'Dirichlet(α+counts)', 'αᵢ += countᵢ'],
              ['InvGamma(α, β)', 'Normal(μ, σ²)', 'InvGamma(α+n/2, ...)', 'Variância com μ fixo'],
            ].map(([a, b, c, d]) => (
              <tr key={a}>
                <td style={S.td}>{a}</td>
                <td style={S.td}>{b}</td>
                <td style={S.td}>{c}</td>
                <td style={{ ...S.td, fontSize: '0.8rem', fontFamily: 'monospace' }}>{d}</td>
              </tr>
            ))}
          </tbody>
        </table>
        <h3 style={S.h3}>Árvore de decisão: Quando usar Bayesiano?</h3>
        <div style={S.diagram}>
          <svg viewBox="0 0 820 270" style={{ width: '100%', height: 'auto', display: 'block', margin: '0 auto' }}>
            {/* Root */}
            <rect x="310" y="10" width="200" height="38" rx="8" fill="rgba(249,115,22,0.10)" stroke={color} strokeWidth="1.5" />
            <text x="410" y="26" textAnchor="middle" fontSize="11" fontWeight="700" fill={color}>Novo problema de inferência</text>
            <text x="410" y="42" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">Qual abordagem usar?</text>
            {/* Arrow down */}
            <line x1="410" y1="48" x2="410" y2="72" stroke="var(--text-secondary)" strokeWidth="1.5" />
            {/* Q1 */}
            <rect x="280" y="72" width="260" height="34" rx="6" fill="var(--bg-secondary)" stroke="var(--text-secondary)" strokeWidth="1" />
            <text x="410" y="93" textAnchor="middle" fontSize="11" fill="var(--text-primary)">Prior knowledge disponível?</text>
            {/* Yes branch — left */}
            <line x1="280" y1="89" x2="140" y2="135" stroke="var(--text-secondary)" strokeWidth="1.5" />
            <text x="175" y="118" fontSize="9" fill="#f97316" textAnchor="middle">Sim</text>
            <rect x="30" y="135" width="200" height="34" rx="6" fill="rgba(249,115,22,0.10)" stroke="#f97316" strokeWidth="1.5" />
            <text x="130" y="156" textAnchor="middle" fontSize="10" fontWeight="600" fill="#f97316">Bayesiano (com prior info)</text>
            {/* No branch — right */}
            <line x1="540" y1="89" x2="660" y2="135" stroke="var(--text-secondary)" strokeWidth="1.5" />
            <text x="605" y="128" fontSize="9" fill="var(--text-secondary)" textAnchor="middle">Não</text>
            {/* Q2 */}
            <rect x="560" y="135" width="200" height="34" rx="6" fill="var(--bg-secondary)" stroke="var(--text-secondary)" strokeWidth="1" />
            <text x="660" y="156" textAnchor="middle" fontSize="10" fill="var(--text-primary)">n pequeno ou IC direto?</text>
            {/* Q2 Yes */}
            <line x1="560" y1="152" x2="460" y2="200" stroke="var(--text-secondary)" strokeWidth="1.5" />
            <text x="520" y="185" fontSize="9" fill="#f97316" textAnchor="middle">Sim</text>
            <rect x="360" y="200" width="195" height="34" rx="6" fill="rgba(249,115,22,0.10)" stroke={color} strokeWidth="1.5" />
            <text x="457" y="221" textAnchor="middle" fontSize="10" fontWeight="600" fill={color}>Bayesiano (prior vago)</text>
            {/* Q2 No */}
            <line x1="740" y1="168" x2="740" y2="200" stroke="var(--text-secondary)" strokeWidth="1.5" />
            <text x="758" y="185" fontSize="9" fill="var(--text-secondary)" textAnchor="middle">Não</text>
            <rect x="645" y="200" width="160" height="34" rx="6" fill="rgba(249,115,22,0.08)" stroke="var(--text-secondary)" strokeWidth="1.5" />
            <text x="725" y="221" textAnchor="middle" fontSize="10" fontWeight="600" fill="var(--text-secondary)">Frequentista OK</text>
            {/* Middle path down */}
            <line x1="410" y1="106" x2="410" y2="135" stroke="var(--text-secondary)" strokeWidth="1.5" />
            <rect x="340" y="135" width="140" height="34" rx="6" fill="var(--bg-secondary)" stroke="var(--text-secondary)" strokeWidth="1" />
            <text x="410" y="156" textAnchor="middle" fontSize="10" fill="var(--text-primary)">Hierárquico?</text>
            <line x1="410" y1="169" x2="410" y2="200" stroke="var(--text-secondary)" strokeWidth="1.5" />
            <text x="422" y="188" fontSize="9" fill="#f97316">Sim</text>
          </svg>
        </div>
        <h3 style={S.h3}>Ecossistema de ferramentas</h3>
        <table style={S.table}>
          <thead>
            <tr>
          ...
          <tbody>
            {[
              ['PyMC', 'Python', 'MCMC geral, modelos hierárquicos, API intuitiva'],
              ['Stan / CmdStanPy', 'Python/R/Julia', 'Modelos complexos, HMC robusto, produção'],
              ['Pyro / NumPyro', 'Python (PyTorch/JAX)', 'Deep learning + Bayes, large scale, GPU'],
              ['ArviZ', 'Python', 'Diagnósticos, visualizações, comparação de modelos'],
              ['brms', 'R', 'Modelos multilevel com sintaxe tipo lme4'],
              ['JAGS', 'R (via rjags)', 'Gibbs sampling, modelos gráficos clássicos'],
            ].map(([a, b, c]) => (
              <tr key={a}>
                <td style={S.td}><strong>{a}</strong></td>
                <td style={{ ...S.td, fontFamily: 'monospace' }}>{b}</td>
        ...
        <div style={S.highlight}>
          <p style={S.p}><strong>Princípios fundamentais para levar:</strong></p>
          <p style={S.p}>
            1. Posterior ∝ Likelihood × Prior — a equação central de tudo.
          </p>
          <p style={S.p}>
            2. Com n grande e priors razoáveis, Bayes e frequentista convergem — a filosofia importa mais do que pensa em n pequeno.
          </p>
          <p style={S.p}>
            3. O intervalo de credibilidade tem a interpretação que erroneamente se dá ao IC frequentista — use Bayes quando precisar de P(θ ∈ [a,b] | dados).
          </p>
          <p style={{ ...S.p, marginBottom: 0 }}>
            4. Modelos hierárquicos são a killer app do Bayes — partial pooling bate complete pooling e no pooling em dados reais com grupos de tamanhos variados.
          </p>
        </div>
      </section>
    </div>
