...
const GADiagram = () => (
  <div style={{ ...S.diagram, textAlign: 'center' }}>
    ...
    <svg viewBox="0 0 580 130" style={{ maxWidth: '100%', height: 'auto' }}>
      <defs>
        <marker id="arr-ga" markerWidth="6" markerHeight="6" refX="3" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill="#4a9eed"/></marker>
      </defs>
      {[
        { x: 15, label: 'População\nInicial', sub: 'N soluções\naleatórias', c: '#4a9eed' },
        { x: 130, label: 'Avaliação', sub: 'f(x) para\ncada indivíduo', c: '#38bdf8' },
        { x: 245, label: 'Selecção', sub: 'pais com\nmelhor fitness', c: '#bae6fd' },
        ...
        <g key={x}>
          ...
          {x < 475 && <path d={`M ${x + 90},47 L ${x + 115},47`} fill="none" stroke="#4a9eed" strokeWidth="1.5" markerEnd="url(#arr-ga)"/>}
        </g>
      ))}
      {/* Cycle back arrow */}
      <path d="M 565,75 Q 565,110 40,110 Q 15,110 15,75" fill="none" stroke="#4a9eed" strokeWidth="1.5" strokeDasharray="4,2" markerEnd="url(#arr-ga)"/>
            ...
            <table style={S.table}>
              ...
              <tbody>
                {[
                  ...
                  <tr key={b}><td style={{ ...S.td, fontWeight: 600, color: '#4a9eed' }}>{b}</td><td style={S.td}>{c}</td><td style={{ ...S.td, color: 'var(--text-secondary)', fontSize: '0.85rem' }}>{p}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
            ...
            <table style={S.table}>
              ...
              <tbody>
                {[
                  ...
                  <tr key={p}><td style={{ ...S.td, fontWeight: 600, color: '#4a9eed' }}>{p}</td><td style={{ ...S.td, fontFamily: 'monospace' }}>{v}</td><td style={{ ...S.td, color: '#4a9eed', fontSize: '0.85rem' }}>{l}</td><td style={{ ...S.td, color: '#4a9eed', fontSize: '0.85rem' }}>{h}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
